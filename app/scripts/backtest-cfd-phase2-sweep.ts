...
-----------------------------------------------*/
/**
 * File: backtest-cfd-phase2-sweep.ts
 *
 * Description:
 * Phase 2 optimization sweep for 5m CFD triggers. Compares
 * MA+Bollinger directional-close variants, RRanjan combinations,
 * RRanjan pre-arm logic, and a standalone RRanjan baseline
 * against the weekly Flagship bias.
 *
 * Run:
 *   npx tsx scripts/backtest-cfd-phase2-sweep.ts
 */
/*-----------------------------------------------
...
type VariantKind =
  | "DIRCLOSE"
  | "DIRCLOSE_RRANJAN"
  | "DIRCLOSE_RRANJAN_PREARM"
  | "DIRCLOSE_ENGULF"
  | "RRANJAN_STANDALONE";
type BbMode = "FIXED" | "ADAPTIVE" | "NONE";
...
type SummaryRow = {
  ...
  highWinLowMaeScore: number;
};

type OverlapStats = {
  samePairSessionWeekCount: number;
  samePairSessionWeekPct: number;
  exactEntryCount: number;
  exactEntryPct: number;
};
...
const LOW_SAMPLE_TOTAL_TRADES = 200;
const LOW_SAMPLE_ASSET_TRADES = 50;
...
const TRIGGER_CONFIGS: TriggerConfig[] = [
  { id: "dirclose__bb_adaptive", kind: "DIRCLOSE", bbMode: "ADAPTIVE" },
  { id: "dirclose__bb2_0_all", kind: "DIRCLOSE", bbMode: "FIXED", bbStdDev: 2.0 },
  { id: "dirclose__bb2_5_all", kind: "DIRCLOSE", bbMode: "FIXED", bbStdDev: 2.5 },
  { id: "dirclose_rranjan__bb2_0", kind: "DIRCLOSE_RRANJAN", bbMode: "FIXED", bbStdDev: 2.0 },
  { id: "dirclose_rranjan__bb2_5", kind: "DIRCLOSE_RRANJAN", bbMode: "FIXED", bbStdDev: 2.5 },
  { id: "dirclose_rranjan__bb_adaptive", kind: "DIRCLOSE_RRANJAN", bbMode: "ADAPTIVE" },
  {
    id: "dirclose_rranjan_prearm__bb2_0",
    kind: "DIRCLOSE_RRANJAN_PREARM",
    bbMode: "FIXED",
    bbStdDev: 2.0,
    prearmLookback: 6,
  },
  {
    id: "dirclose_rranjan_prearm__bb2_5",
    kind: "DIRCLOSE_RRANJAN_PREARM",
    bbMode: "FIXED",
    bbStdDev: 2.5,
    prearmLookback: 6,
  ...
  {
    id: "dirclose_rranjan_prearm__bb_adaptive",
    kind: "DIRCLOSE_RRANJAN_PREARM",
    bbMode: "ADAPTIVE",
    prearmLookback: 6,
  },
  { id: "dirclose_engulf__bb2_0", kind: "DIRCLOSE_ENGULF", bbMode: "FIXED", bbStdDev: 2.0 },
  { id: "dirclose_engulf__bb2_5", kind: "DIRCLOSE_ENGULF", bbMode: "FIXED", bbStdDev: 2.5 },
  { id: "rranjan_standalone", kind: "RRANJAN_STANDALONE", bbMode: "NONE" },
];
...
function buildBbIndicatorPack(candles: OhlcCandle[], bbStdDev: number): IndicatorPack {
  ...
  const bbUpper = bbMid.map((mid, index) =>
    mid === null || bbStd[index] === null ? null : mid + bbStdDev * (bbStd[index] ?? 0),
  );
...
function getAdaptiveBBStdDev(assetClass: AssetClass) {
  switch (assetClass) {
    case "crypto":
      return 2.5;
    case "fx":
...
function isRranjanZone(direction: Exclude<Direction, "NEUTRAL">, value: number | null) {
  if (value === null) return false;
  return direction === "LONG" ? value < OVERSOLD_LEVEL : value > OVERBOUGHT_LEVEL;
}

function hasRecentRranjanArm(
  direction: Exclude<Direction, "NEUTRAL">,
  ...
  for (let i = index - 1; i >= start; i -= 1) {
    if (isRranjanZone(direction, triggerLine[i])) return true;
  }
  return false;
}

function getVariantBbStdDev(config: TriggerConfig, assetClass: AssetClass) {
  if (config.bbMode === "ADAPTIVE") return getAdaptiveBBStdDev(assetClass);
  if (config.bbMode === "FIXED") return config.bbStdDev ?? 2.0;
  return null;
}
...
function findTriggerEntry(
  ...
  direction: Exclude<Direction, "NEUTRAL">,
  candles: OhlcCandle[],
  indicatorPack: IndicatorPack | null,
  triggerLine: Array<number | null>,
  ...
  for (let i = 0; i < candles.length; i += 1) {
    ...
    const triggerValue = triggerLine[i];

    if (config.kind === "RRANJAN_STANDALONE") {
      if (!isRranjanZone(direction, triggerValue)) continue;
      if (!isEngulfing(direction, candles[i - 1], candle)) continue;
      return {
        entryTs: closeTs,
        entryPrice: candle.close,
        signalValue: triggerValue ?? null,
        ma200: null,
        bollingerMid: null,
        bollingerUpper: null,
        bollingerLower: null,
    ...
    if (!indicatorPack) continue;
    const ma = indicatorPack.sma200[i];
    const bbMid = indicatorPack.bbMid[i];
    const bbUpper = indicatorPack.bbUpper[i];
    const bbLower = indicatorPack.bbLower[i];
    ...
    const directionalClose = isDirectionalClose(direction, candle);
    const engulfing = isEngulfing(direction, candles[i - 1], candle);
    const sameCandleRranjan = isRranjanZone(direction, triggerValue);
    const prearmed = hasRecentRranjanArm(direction, triggerLine, i, config.prearmLookback ?? 0);

    const baseOk = trendOk && touchedBand && directionalClose;
    if (!baseOk) continue;

    const qualifies =
      config.kind === "DIRCLOSE" ||
      (config.kind === "DIRCLOSE_RRANJAN" && sameCandleRranjan) ||
      (config.kind === "DIRCLOSE_RRANJAN_PREARM" && prearmed) ||
      (config.kind === "DIRCLOSE_ENGULF" && engulfing);
...
function summarizeTrades(trades: TriggerTrade[]): SummaryRow {
  ...
  const weekMfes = trades
    ...
    .filter((value): value is number => value !== null);
  const avgSessionMaePct = round(safeAverage(sessionMaes), 4);
  ...
  return {
    ...
    winRateSessionClosePct: winRate,
    ...
    highWinLowMaeScore: avgSessionMaePct === 0 ? 0 : round(winRate / avgSessionMaePct, 4),
  };
}
...
function buildTradeSetKeys(trades: TriggerTrade[]) {
  const pairSessionWeek = new Set<string>();
  const exactEntry = new Set<string>();
  for (const trade of trades) {
    pairSessionWeek.add(`${trade.weekOpenUtc}|${trade.session}|${trade.pair}`);
    exactEntry.add(`${trade.weekOpenUtc}|${trade.session}|${trade.pair}|${trade.entryTimeUtc}`);
  }
  return { pairSessionWeek, exactEntry };
}

function computeOverlap(trades: TriggerTrade[], baselineTrades: TriggerTrade[]): OverlapStats {
  if (trades.length === 0 || baselineTrades.length === 0) {
    return {
      samePairSessionWeekCount: 0,
      samePairSessionWeekPct: 0,
      exactEntryCount: 0,
      exactEntryPct: 0,
    };
  }
  const baselineKeys = buildTradeSetKeys(baselineTrades);
  let samePairSessionWeekCount = 0;
  let exactEntryCount = 0;
  for (const trade of trades) {
    if (baselineKeys.pairSessionWeek.has(`${trade.weekOpenUtc}|${trade.session}|${trade.pair}`)) {
      samePairSessionWeekCount += 1;
    }
    if (baselineKeys.exactEntry.has(`${trade.weekOpenUtc}|${trade.session}|${trade.pair}|${trade.entryTimeUtc}`)) {
      exactEntryCount += 1;
    }
  }
  return {
    samePairSessionWeekCount,
    samePairSessionWeekPct: round((samePairSessionWeekCount / trades.length) * 100, 2),
    exactEntryCount,
    exactEntryPct: round((exactEntryCount / trades.length) * 100, 2),
  };
}
...
async function main() {
  ...
  const indicatorPacksByPair = new Map<string, Map<number, IndicatorPack>>();
  ...
  await runWithConcurrency(pairUniverse, FETCH_CONCURRENCY, async (pairInfo) => {
    ...
    try {
      ...
      indicatorPacksByPair.set(
        key,
        new Map([
          [2.0, buildBbIndicatorPack(candles, 2.0)],
          [2.5, buildBbIndicatorPack(candles, 2.5)],
        ]),
      );
    } catch (error) {
  ...
  const variantResults: Array<{
    ...
    bbMode: BbMode;
    bbStdDev: number | null;
    prearmLookback: number | null;
    trades: TriggerTrade[];
    overall: SummaryRow;
    overallFxCryptoOnly: SummaryRow;
    byAssetClass: Array<Record<string, unknown>>;
    bySession: Array<Record<string, unknown>>;
    byDirection: Array<Record<string, unknown>>;
    byPair: Array<Record<string, unknown>>;
    sampleTrades: TriggerTrade[];
  }> = [];
  ...
  for (const config of TRIGGER_CONFIGS) {
    ...
    for (const weekOpenUtc of targetWeeks) {
      ...
      for (const pairInfo of pairUniverse) {
        ...
        const triggerLine = rranjanByPair.get(key);
        const indicatorMap = indicatorPacksByPair.get(key);
        if (!candles || !triggerLine) continue;

        const bbStdDev = getVariantBbStdDev(config, pairInfo.assetClass);
        const indicatorPack = bbStdDev === null ? null : indicatorMap?.get(bbStdDev) ?? null;
        ...
        for (const sessionWindow of sessionWindows) {
          ...
          trades.push({
            ...
            signalValue: found.signalValue === null ? null : round(found.signalValue, 4),
            ma200: found.ma200 === null ? null : round(found.ma200),
            bollingerMid: found.bollingerMid === null ? null : round(found.bollingerMid),
            bollingerUpper: found.bollingerUpper === null ? null : round(found.bollingerUpper),
            bollingerLower: found.bollingerLower === null ? null : round(found.bollingerLower),
            sessionMetrics,
            weekMetrics,
          });
        }
    ...
    variantResults.push({
      ...
      kind: config.kind,
      bbMode: config.bbMode,
      bbStdDev: config.bbMode === "FIXED" ? config.bbStdDev ?? null : config.bbMode === "ADAPTIVE" ? -1 : null,
      prearmLookback: config.prearmLookback ?? null,
      trades,
      overall,
      overallFxCryptoOnly: summarizeTrades(trades.filter((trade) => trade.assetClass !== "indices")),
      ...
      JSON.stringify(
        {
          ...
          winRateSessionClosePct: overall.winRateSessionClosePct,
          avgWeekReturnPct: overall.avgWeekReturnPct,
          avgWeekMaePct: overall.avgWeekMaePct,
        },
        null,
        2,
      ),
  ...
  const baselineDirclose = variantResults.find((variant) => variant.variantId === "dirclose__bb2_0_all");
  const baselineRranjan = variantResults.find((variant) => variant.variantId === "rranjan_standalone");
  if (!baselineDirclose || !baselineRranjan) {
    throw new Error("Expected dirclose__bb2_0_all and rranjan_standalone variants to exist.");
  }

  const finalVariants = variantResults.map((variant) => {
    const lowSampleWarnings: string[] = [];
    if (variant.overall.trades < LOW_SAMPLE_TOTAL_TRADES) {
      lowSampleWarnings.push(`total trades below ${LOW_SAMPLE_TOTAL_TRADES}`);
    }
    for (const row of variant.byAssetClass as Array<{ assetClass: AssetClass; trades: number }>) {
      if (row.trades < LOW_SAMPLE_ASSET_TRADES) {
        lowSampleWarnings.push(`${row.assetClass} trades below ${LOW_SAMPLE_ASSET_TRADES}`);
      }
    }
    ...
    return {
      ...
      kind: variant.kind,
      bbMode: variant.bbMode,
      bbStdDev: variant.bbMode === "ADAPTIVE" ? "adaptive" : variant.bbStdDev,
      prearmLookback: variant.prearmLookback,
      overall: variant.overall,
      overallFxCryptoOnly: variant.overallFxCryptoOnly,
      byAssetClass: variant.byAssetClass,
      bySession: variant.bySession,
      byDirection: variant.byDirection,
      byPair: variant.byPair,
      sampleTrades: variant.sampleTrades,
      tradeCountReduction: {
        vsDircloseBb20All:
          baselineDirclose.overall.trades === 0
            ? 0
            : round(
                ((baselineDirclose.overall.trades - variant.overall.trades) / baselineDirclose.overall.trades) * 100,
                2,
              ),
        vsRranjanStandalone:
          baselineRranjan.overall.trades === 0
            ? 0
            : round(
                ((baselineRranjan.overall.trades - variant.overall.trades) / baselineRranjan.overall.trades) * 100,
                2,
              ),
      },
      overlapWithRranjanStandalone: computeOverlap(variant.trades, baselineRranjan.trades),
      lowSampleWarnings,
    };
  });

  const ranking = [...finalVariants]
    .sort((a, b) => {
    ...
    .map((variant, index) => ({
      ...
      kind: variant.kind,
      bbMode: variant.bbMode,
      bbStdDev: variant.bbStdDev,
      prearmLookback: variant.prearmLookback,
      ...variant.overall,
      overlapWithRranjanStandalone: variant.overlapWithRranjanStandalone,
      tradeCountReduction: variant.tradeCountReduction,
      lowSampleWarnings: variant.lowSampleWarnings,
    }));

  const highWinLowMaeRanking = [...finalVariants]
    .sort((a, b) => {
      if (b.overall.highWinLowMaeScore !== a.overall.highWinLowMaeScore) {
        return b.overall.highWinLowMaeScore - a.overall.highWinLowMaeScore;
      }
      if (b.overall.winRateSessionClosePct !== a.overall.winRateSessionClosePct) {
        return b.overall.winRateSessionClosePct - a.overall.winRateSessionClosePct;
      }
      return a.overall.avgSessionMaePct - b.overall.avgSessionMaePct;
    })
    .map((variant, index) => ({
      rank: index + 1,
      variantId: variant.variantId,
      highWinLowMaeScore: variant.overall.highWinLowMaeScore,
      winRateSessionClosePct: variant.overall.winRateSessionClosePct,
      avgSessionMaePct: variant.overall.avgSessionMaePct,
      trades: variant.overall.trades,
      lowSampleWarnings: variant.lowSampleWarnings,
    }));

  const overlap = finalVariants.map((variant) => ({
    variantId: variant.variantId,
    ...variant.overlapWithRranjanStandalone,
  }));
  ...
  const report = {
    ...
    config: {
      ...
      bbLength: BB_LENGTH,
      rranjan: {
        stochLength: STOCHASTIC_K_LENGTH,
        kSmoothing: STOCHASTIC_K_SMOOTHING,
        dSmoothing: STOCHASTIC_D_SMOOTHING,
      ...
      lowSampleThresholds: {
        totalTrades: LOW_SAMPLE_TOTAL_TRADES,
        assetClassTrades: LOW_SAMPLE_ASSET_TRADES,
      },
    },
    ranking,
    ...
    missingPairs,
    notes: [
      "Phase 2 sweep compares MA+BB directional-close variants, RRanjan combinations, RRanjan pre-arm logic, stacked confirmations, and a standalone RRanjan baseline.",
      "drawdownAdjustedWeekScore = avgWeekReturnPct / avgWeekMaePct.",
      "highWinLowMaeScore = sessionCloseWinRatePct / avgSessionMaePct.",
      "overallFxCryptoOnly excludes indices to make the trigger read cleaner given weak index behavior in prior passes.",
      "Trade-count reduction and overlap vs standalone RRanjan help show whether stacked filters are additive or just more selective.",
    ],
  };

  mkdirSync(REPORTS_DIR, { recursive: true });
  const timestamp = DateTime.utc().toFormat("yyyyLLdd-HHmmss");
  const reportPath = path.join(REPORTS_DIR, `cfd-phase2-sweep-${timestamp}.json`);
  const latestPath = path.join(REPORTS_DIR, "cfd-phase2-sweep-latest.json");
  writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf8");
  writeFileSync(latestPath, JSON.stringify(report, null, 2), "utf8");

  console.log("CFD Phase 2 sweep complete.");
  console.log(
    JSON.stringify(
      {
        topByDrawdownAdjusted: ranking.slice(0, 5),
        topByHighWinLowMae: highWinLowMaeRanking.slice(0, 5),
        reportPath,
        latestPath,
...
main().catch((error) => {
  console.error("backtest-cfd-phase2-sweep failed:", error);
  process.exitCode = 1;
});
