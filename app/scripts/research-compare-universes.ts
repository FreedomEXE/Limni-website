...
async function main() {
  ensureEnv();
  const raw = JSON.parse(readFileSync("Local Environment/data/research_universe.json", "utf-8")) as UniverseReport;
  ...
  const outPath = `app/reports/research-universe-compare-${DateTime.utc().toISODate()}.json`;
