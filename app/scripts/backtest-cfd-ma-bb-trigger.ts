...
-----------------------------------------------*/
/**
 * File: backtest-cfd-ma-bb-trigger.ts
 *
 * Description:
 * First-pass CFD trigger research for a 5m pullback system aligned to the
 * weekly Flagship bias. Tests:
 * - LONG: close above 200 SMA and candle touches/pierces lower Bollinger band
 * - SHORT: close below 200 SMA and candle touches/pierces upper Bollinger band
 * Entry is on the trigger candle close. First trade only per pair per
 * eligible session. Reports passive session-close and week-close outcomes.
 *
 * Run:
 *   npx tsx scripts/backtest-cfd-ma-bb-trigger.ts
 */
/*-----------------------------------------------
...
function computeTradeMetrics(
  ...
  if (!exitCandle) {
    return { exitPrice: null, returnPct: null, maePct: null, mfePct: null };
  }
...
function findTriggerEntry(
  ...
  for (let i = 0; i < candles.length; i += 1) {
    ...
    const qualifies =
      direction === "LONG"
        ? candle.close > ma && candle.low <= bbLower
        : candle.close < ma && candle.high >= bbUpper;
...
async function main() {
  ...
  const report = {
    ...
    config: {
      ...
      entryRule: {
        LONG: "5m close above 200 SMA and candle low touches/pierces lower Bollinger band; enter on candle close",
        SHORT: "5m close below 200 SMA and candle high touches/pierces upper Bollinger band; enter on candle close",
      },
    ...
    notes: [
      "This is a first-pass raw edge study for a 5m SMA+Bollinger trigger aligned to the weekly Flagship bias.",
      "No stop loss or take profit is applied yet. Session-close and week-close metrics are reported to evaluate follow-through before designing exits.",
      "If this shows promise, the next pass should compare pure MA+BB against MA+BB+RRanjan variants.",
    ],
  };

  mkdirSync(REPORTS_DIR, { recursive: true });
  const timestamp = DateTime.utc().toFormat("yyyyLLdd-HHmmss");
  const reportPath = path.join(REPORTS_DIR, `cfd-ma-bb-trigger-backtest-${timestamp}.json`);
  const latestPath = path.join(REPORTS_DIR, "cfd-ma-bb-trigger-backtest-latest.json");
  writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf8");
  writeFileSync(latestPath, JSON.stringify(report, null, 2), "utf8");

  console.log("CFD MA+BB trigger backtest complete.");
  console.log(JSON.stringify({ overall, reportPath, latestPath, missingPairs: missingPairs.length }, null, 2));
}

main().catch((error) => {
  console.error("backtest-cfd-ma-bb-trigger failed:", error);
  process.exitCode = 1;
});
