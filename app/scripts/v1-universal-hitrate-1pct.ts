...
async function main() {
  ...
  const compare = JSON.parse(
    fs.readFileSync("app/reports/eightcap-3k-5week-floor-clamped-compare-latest.json", "utf8"),
  ) as { weeks: string[] };
  ...
  const jsonPath = `app/reports/v1-universal-hitrate-1pct-${stamp}.json`;
  const latestJsonPath = "app/reports/v1-universal-hitrate-1pct-latest.json";
  const mdPath = `app/reports/v1-universal-hitrate-1pct-${stamp}.md`;
  const latestMdPath = "app/reports/v1-universal-hitrate-1pct-latest.md";
