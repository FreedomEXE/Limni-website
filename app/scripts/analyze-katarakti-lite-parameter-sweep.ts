...
const LITE_REPORT_PATH = path.join(process.cwd(), "app", "reports", "katarakti-lite-parameter-sweep-latest.txt");
const LITE_JSON_REPORT_PATH = path.join(process.cwd(), "app", "reports", "katarakti-lite-parameter-sweep-latest.json");
...
async function mainLiteParameterSweep() {
  loadDotEnv();
  ...
  const dwellValues = Array.from(new Set(parseNumberList(LITE_SWEEP_DWELLS).map((x) => Math.max(0, Math.round(x))))).sort((a, b) => a - b);
  const closeLocValues = Array.from(new Set(parseNumberList(LITE_SWEEP_CLOSE_LOCS).map((x) => Math.max(0.05, Math.min(0.95, Number(x.toFixed(2))))))).sort((a, b) => a - b);
  const atrFloorTokens = LITE_SWEEP_ATR_FLOORS.split(",").map((x) => x.trim()).filter((x) => x.length > 0);
  const atrFloorValues = Array.from(new Set(
    atrFloorTokens.map((token) => {
      if (token.toLowerCase() === "off" || token === "0") return null;
  ...
  for (const dwell of dwellValues) {
    for (const closeLoc of closeLocValues) {
      for (const atrFloor of atrFloorValues) {
        const atrLabel = atrFloor === null ? "off" : `${atrFloor.toFixed(2)}x`;
        variants.push({
          id: `lite_d${dwell}_c${closeLoc.toFixed(2).replace(".", "")}_a${atrFloor === null ? "off" : atrFloor.toFixed(2).replace(".", "")}`,
          label: `Lite Re-entry d=${dwell} c=${closeLoc.toFixed(2)} atrFloor=${atrLabel}`,
          entryRule: "reentry",
          handshakeConfig: handshakeOff,
          sweepThresholdPct: LITE_SWEEP_THRESHOLD_PCT,
          dwellMin: dwell,
          closeLocationPct: closeLoc,
          atrSweepFloorX: atrFloor,
        });
      }
    }
  }
  ...
  lines.push(`Dwell grid: ${LITE_SWEEP_DWELLS}`);
  lines.push(`Close-location grid: ${LITE_SWEEP_CLOSE_LOCS}`);
  lines.push(`ATR floor grid: ${LITE_SWEEP_ATR_FLOORS}`);
  lines.push("");
  ...
  lines.push("Combined parameter sweep summary:");
  lines.push("| Variant | Trades | WR% | Total PnL | PF | Max DD% | Total Return % |");
  lines.push("| --- | ---: | ---: | ---: | ---: | ---: | ---: |");
  for (const r of [...results].sort((a, b) => b.totalReturnPct - a.totalReturnPct || a.maxDrawdownPct - b.maxDrawdownPct)) {
  ...
  const nonBaseline = results.filter((r) => r.variant.entryRule === "reentry");
  const topByReturn = [...nonBaseline].sort((a, b) => b.totalReturnPct - a.totalReturnPct).slice(0, 10);
  const topByRiskAdj = [...nonBaseline]
    .sort((a, b) => (b.maxDrawdownPct > 0 ? b.totalReturnPct / b.maxDrawdownPct : Number.POSITIVE_INFINITY) - (a.maxDrawdownPct > 0 ? a.totalReturnPct / a.maxDrawdownPct : Number.POSITIVE_INFINITY))
    .slice(0, 10);

  lines.push("Top 10 lite variants by return:");
  lines.push("| Rank | Variant | Dwell | CloseLoc | ATR floor | Trades | Return % | Max DD% | PF |");
  lines.push("| ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
  topByReturn.forEach((r, idx) => {
    lines.push(`| ${idx + 1} | ${r.variant.id} | ${r.variant.dwellMin} | ${r.variant.closeLocationPct?.toFixed(2) ?? "off"} | ${r.variant.atrSweepFloorX === null ? "off" : `${r.variant.atrSweepFloorX.toFixed(2)}x`} | ${r.trades.length} | ${r.totalReturnPct >= 0 ? "+" : ""}${r.totalReturnPct.toFixed(2)}% | ${r.maxDrawdownPct.toFixed(2)}% | ${fmtPf(r.profitFactor)} |`);
  });
  lines.push("");

  lines.push("Top 10 lite variants by return/maxDD:");
  lines.push("| Rank | Variant | Dwell | CloseLoc | ATR floor | Trades | Return % | Max DD% | Return/DD |");
  lines.push("| ---: | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
  topByRiskAdj.forEach((r, idx) => {
    const ratio = r.maxDrawdownPct > 0 ? r.totalReturnPct / r.maxDrawdownPct : Number.POSITIVE_INFINITY;
    lines.push(`| ${idx + 1} | ${r.variant.id} | ${r.variant.dwellMin} | ${r.variant.closeLocationPct?.toFixed(2) ?? "off"} | ${r.variant.atrSweepFloorX === null ? "off" : `${r.variant.atrSweepFloorX.toFixed(2)}x`} | ${r.trades.length} | ${r.totalReturnPct >= 0 ? "+" : ""}${r.totalReturnPct.toFixed(2)}% | ${r.maxDrawdownPct.toFixed(2)}% | ${Number.isFinite(ratio) ? ratio.toFixed(2) : "INF"} |`);
  });
  lines.push("");
...
mainLiteParameterSweep().catch(async (error) => {
  console.error("analyze-katarakti-lite-parameter-sweep failed:", error);
  try {
    await getPool().end();
  } catch {
    // ignore
