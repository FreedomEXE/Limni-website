...
async function readJsonFile<T>(relativePath: string): Promise<T | null> {
  try {
    const absolutePath = resolveRepoCandidatePath(relativePath);
...
function resolveRepoCandidatePath(candidatePath: string) {
  if (path.isAbsolute(candidatePath)) return candidatePath;
  if (/^reports[\\/]/.test(candidatePath)) return appPath(candidatePath);
