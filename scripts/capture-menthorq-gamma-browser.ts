...
function parseArgs(): CliConfig {
  ...
  const outCsv = byKey.get("out-csv")?.trim() || "reports/bias-gate/menthorq-gamma-daily.csv";
  const outDir = byKey.get("out-dir")?.trim() || "reports/bias-gate/menthorq-captures";
  ...
  const userDataDir = byKey.get("user-data-dir")?.trim() || ".cache/playwright-menthorq";
