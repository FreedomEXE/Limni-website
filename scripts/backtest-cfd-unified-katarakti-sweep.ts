...
-----------------------------------------------*/
/**
 * File: backtest-cfd-unified-katarakti-sweep.ts
 *
 * Description:
 * Unified CFD entry sweep that compares:
 * - Katarakti-style sweep + rejection + displacement entries
 * - MA+BB directional-close control
 *
 * against the same OANDA M5 dataset, weekly COT bias, weekly basket
 * handshake, and COT-date-aware gate modes.
 *
 * Run:
 *   npx tsx scripts/backtest-cfd-unified-katarakti-sweep.ts
 */
/*-----------------------------------------------
...
type TriggerTrade = {
  ...
  wickStopPrice: number | null;
  wickStopDistancePct: number | null;
  wickStopHitSession: boolean;
  wickStopHitWeek: boolean;
  wickStopReturnPctSession: number | null;
  wickStopReturnPctWeek: number | null;
...
const TARGET_WEEKS_COUNT = Number(process.env.CFD_UNIFIED_KATARAKTI_BACKTEST_WEEKS ?? "8");
const WARMUP_DAYS = Number(process.env.CFD_UNIFIED_KATARAKTI_WARMUP_DAYS ?? "14");
const FETCH_CONCURRENCY = Number(process.env.CFD_UNIFIED_KATARAKTI_FETCH_CONCURRENCY ?? "4");
const MA_LENGTH = Number(process.env.CFD_UNIFIED_KATARAKTI_MA_LENGTH ?? "200");
const BB_LENGTH = Number(process.env.CFD_UNIFIED_KATARAKTI_BB_LENGTH ?? "20");
const PAIR_FILTER = new Set(
  (process.env.CFD_UNIFIED_KATARAKTI_PAIR_FILTER ?? "")
    .split(",")
    .map((value) => value.trim().toUpperCase())
    .filter(Boolean),
);

const GATE_MODES: GateMode[] = ["UNGATED", "FROZEN", "LIVE"];

const BASE_VARIANTS: BaseVariant[] = [
  { id: "sweep_010__no_hs", strategy: "SWEEP", sweepThresholdPct: 0.1, handshakeThresholdPct: null },
  { id: "sweep_020__no_hs", strategy: "SWEEP", sweepThresholdPct: 0.2, handshakeThresholdPct: null },
  { id: "sweep_030__no_hs", strategy: "SWEEP", sweepThresholdPct: 0.3, handshakeThresholdPct: null },
  { id: "sweep_010__w60", strategy: "SWEEP", sweepThresholdPct: 0.1, handshakeThresholdPct: 60 },
  { id: "sweep_020__w60", strategy: "SWEEP", sweepThresholdPct: 0.2, handshakeThresholdPct: 60 },
  { id: "sweep_030__w60", strategy: "SWEEP", sweepThresholdPct: 0.3, handshakeThresholdPct: 60 },
  {
    id: "sweep_010__w60__disp005",
    strategy: "SWEEP",
    sweepThresholdPct: 0.1,
    handshakeThresholdPct: 60,
    displacementBodyMinOverridePct: 0.05,
    closeZoneOverridePct: 0.30,
  },
  {
    id: "sweep_010__w60__disp010",
    strategy: "SWEEP",
    sweepThresholdPct: 0.1,
    handshakeThresholdPct: 60,
    displacementBodyMinOverridePct: 0.10,
    closeZoneOverridePct: 0.30,
  },
  {
    id: "sweep_010__w60__disp010_nz",
    strategy: "SWEEP",
    sweepThresholdPct: 0.1,
    handshakeThresholdPct: 60,
    displacementBodyMinOverridePct: 0.10,
    closeZoneOverridePct: 1.0,
  },
  {
  ...
  {
    id: "sweep_010__w60__sma_filter",
    strategy: "SWEEP",
    sweepThresholdPct: 0.1,
    handshakeThresholdPct: 60,
    requireSmaFilter: true,
  },
  {
    id: "mabb_dirclose__w60",
    strategy: "MA_BB_CONTROL",
    sweepThresholdPct: null,
    handshakeThresholdPct: 60,
  },
];
...
async function main() {
  ...
  for (const config of VARIANT_CONFIGS) {
    ...
    for (const weekOpenUtc of targetWeeks) {
      ...
      for (const pairInfo of pairUniverse) {
        ...
        for (const window of entryWindows) {
          ...
          const wickStopSession = computeWickStopResult(
            selected.candidate.direction,
            candles,
            selected.candidate.entryTs,
            selected.candidate.entryPrice,
            selected.candidate.wickStopPrice,
            window.entryEnd.toMillis(),
          );
          const wickStopWeek = computeWickStopResult(
            selected.candidate.direction,
            candles,
            selected.candidate.entryTs,
            selected.candidate.entryPrice,
          ...
          trades.push({
            ...
            wickStopHitSession: wickStopSession.hit,
            wickStopHitWeek: wickStopWeek.hit,
            wickStopReturnPctSession: wickStopSession.returnPct,
            wickStopReturnPctWeek: wickStopWeek.returnPct,
            sessionMetrics,
            weekMetrics,
          });
        }
    ...
    const wickStopHitsSession = trades.filter((trade) => trade.wickStopHitSession);
    const wickStopHitsWeek = trades.filter((trade) => trade.wickStopHitWeek);
    const noWickStopWeek = trades.filter((trade) => !trade.wickStopHitWeek);
    ...
    variantOutputs.push({
      ...
      overallFxCryptoOnly: summarizeTrades(fxCryptoOnly),
      ...
      bySession: aggregateBy(trades, (trade) => trade.session)
        ...
        .map((row) => ({ session: row.key as WindowSession, ...row })),
      byDirection: aggregateBy(trades, (trade) => trade.direction)
        .sort((a, b) => String(a.key).localeCompare(String(b.key)))
        .map((row) => ({ direction: row.key as Exclude<Direction, "NEUTRAL">, ...row })),
      byTier: aggregateBy(trades, (trade) => trade.tier)
        .sort((a, b) => String(a.key).localeCompare(String(b.key)))
        .map((row) => ({ tier: row.key as "HIGH" | "MEDIUM", ...row })),
      ...
      wickStopAnalysis: {
        sessionHitRatePct: trades.length === 0 ? 0 : round((wickStopHitsSession.length / trades.length) * 100, 2),
        weekHitRatePct: trades.length === 0 ? 0 : round((wickStopHitsWeek.length / trades.length) * 100, 2),
        avgWeekReturnPctWhenStopHit: round(safeAverage(wickStopHitsWeek.map((trade) => trade.weekMetrics.returnPct).filter((value): value is number => value !== null)), 4),
        avgWeekReturnPctWhenStopNotHit: round(safeAverage(noWickStopWeek.map((trade) => trade.weekMetrics.returnPct).filter((value): value is number => value !== null)), 4),
        winRateWhenStopNotHitPct:
          noWickStopWeek.length === 0
            ? 0
            : round((noWickStopWeek.filter((trade) => (trade.weekMetrics.returnPct ?? 0) > 0).length / noWickStopWeek.length) * 100, 2),
      },
      handshakeStats: {
        averageAgreementPct: round(safeAverage(agreementValues), 2),
        medianAgreementPct: round(median(agreementValues), 2),
        distribution: {
          "0_30": agreementValues.filter((value) => value < 30).length,
          "30_40": agreementValues.filter((value) => value >= 30 && value < 40).length,
  ...
  const ranking = [...variantOutputs]
    ...
    .map((row, index) => ({
      ...
      overallFxCryptoOnly: row.overallFxCryptoOnly,
    }));
  ...
  const report = {
    ...
    config: {
      ...
      gating: {
        UNGATED: "dealer + commercial + weekly sentiment snapshot from performance_snapshots",
      ...
      exits: "passive entry-window close and passive week close marks; wick stop hit analysis recorded separately",
    },
    ranking,
    variants: variantOutputs,
    missingPairs,
    notes: [
      "Commodities are included in the sweep but are not mixed into FX/USD handshake buckets in this pass.",
      "Gate-aware variants use dealer + commercial weekly votes plus historical daily sentiment lock as of session open or candidate entry time.",
      "The unified sweep reports both all-asset summaries and FX+crypto-only summaries because indices/commodities can dilute read-through.",
    ],
  };

  mkdirSync(REPORTS_DIR, { recursive: true });
  const timestamp = DateTime.utc().toFormat("yyyyLLdd-HHmmss");
  const reportPath = path.join(REPORTS_DIR, `cfd-unified-katarakti-sweep-${timestamp}.json`);
  const latestPath = path.join(REPORTS_DIR, "cfd-unified-katarakti-sweep-latest.json");
  writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf8");
  writeFileSync(latestPath, JSON.stringify(report, null, 2), "utf8");

  console.log("CFD unified Katarakti sweep complete.");
  console.log(JSON.stringify({ ranking: ranking.slice(0, 10), reportPath, latestPath, missingPairs: missingPairs.length }, null, 2));
}
main().catch((error) => {
