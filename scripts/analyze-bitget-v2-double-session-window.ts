...
function nyPlusNextAsiaCandleIndicesForRangeDay(
  candles: Candle[],
  rangeDayUtc: string,
  filters: SessionFilters = BASELINE_SESSION_FILTERS,
) {
  const idx: number[] = [];
  const day = DateTime.fromISO(rangeDayUtc, { zone: "utc" });
  if (!day.isValid) return idx;
  const nyDay = day.toISODate();
  const nextAsiaDay = day.plus({ days: 1 }).toISODate();
  if (!nyDay || !nextAsiaDay) return idx;

  for (let i = 0; i < candles.length; i += 1) {
    const candle = candles[i];
    const candleDay = getUtcDateKey(candle.ts);
    if (candleDay === nyDay && filters.isNySessionCandle(candle.ts)) {
      idx.push(i);
      continue;
    }
    if (candleDay === nextAsiaDay && filters.isAsiaSessionCandle(candle.ts)) {
      idx.push(i);
    }
  }
  return idx;
...
type DoubleSessionVariantKey = "baseline" | "double";

type DoubleSessionVariantState = {
  balance: number;
  peak: number;
  maxDd: number;
  tradeId: number;
...
function computeProfitFactor(trades: ClosedTrade[]) {
  let grossWins = 0;
  let grossLosses = 0;
  for (const trade of trades) {
    if (trade.pnl_usd > 0) grossWins += trade.pnl_usd;
    else if (trade.pnl_usd < 0) grossLosses += Math.abs(trade.pnl_usd);
  }
  if (grossLosses === 0) return Number.POSITIVE_INFINITY;
  return grossWins / grossLosses;
}

function summarizeSessionWindow(trades: ClosedTrade[], sessionWindow: SessionWindow) {
  const rows = trades.filter((trade) => trade.session_window === sessionWindow);
  const wins = rows.filter((trade) => trade.pnl_usd > 0).length;
  const pnl = rows.reduce((sum, trade) => sum + trade.pnl_usd, 0);
  const winRate = rows.length ? (wins / rows.length) * 100 : 0;
  return { trades: rows.length, winRatePct: winRate, pnlUsd: pnl };
}

function tradeIdentityKey(trade: ClosedTrade) {
  return [
    trade.week_open_utc,
    trade.symbol,
    trade.direction,
...
function printTradeList(header: string, trades: ClosedTrade[]) {
  console.log(header);
  if (!trades.length) {
    console.log("  (none)");
    return;
  ...
  for (const trade of trades) {
    const outcome = trade.pnl_usd > 0 ? "win" : trade.pnl_usd < 0 ? "loss" : "flat";
    console.log(
      `  ${trade.symbol} ${trade.direction} ${trade.entry_time_utc} ${trade.session_window} ${trade.exit_reason} ${formatUsd(trade.pnl_usd)} ${outcome}`,
    );
  }
}

async function runV2DoubleSessionComparison(params: {
  weekOpens: string[];
  cotHistory: CotSnapshot[];
  fundingBySymbol: Record<CoreSymbol, FundingPoint[]>;
}) {
  const { weekOpens, cotHistory, fundingBySymbol } = params;
  const variants: Array<{
    key: DoubleSessionVariantKey;
    label: string;
    asiaWindowLabel: string;
    useExtendedNy: boolean;
  }> = [
    {
      key: "baseline",
      label: "BASELINE (current V2)",
      asiaWindowLabel: "ASIA_LONDON_RANGE_NY",
      useExtendedNy: false,
    },
    {
      key: "double",
      label: "DOUBLE SESSION (extended NY window)",
      asiaWindowLabel: "ASIA_LONDON_RANGE_NY+ASIA",
      useExtendedNy: true,
    },
  ];

  const state = {} as Record<DoubleSessionVariantKey, DoubleSessionVariantState>;
  ...
  for (const weekOpenUtc of weekOpens) {
    ...
    biasRows.push({
      weekOpenUtc,
      btc: `${biases.BTC.tier}/${biases.BTC.bias}`,
      eth: `${biases.ETH.tier}/${biases.ETH.bias}`,
    });
    ...
    const filters = BASELINE_SESSION_FILTERS;
    const ranges = {
      asiaLondon: {
    ...
    const weeklyEntries = {} as Record<DoubleSessionVariantKey, Record<CoreSymbol, number>>;
    for (const variant of variants) {
      ...
      for (const variant of variants) {
        ...
        const sessionDefinitions: Array<{
        ...
        }> = [
          {
            ...
            entrySession: "NY",
            rangeForSymbol: (symbol) => ranges.asiaLondon[symbol].get(dayUtc),
            sessionIndicesForSymbol: (symbol) => variant.useExtendedNy
              ? nyPlusNextAsiaCandleIndicesForRangeDay(candlesBySymbol[symbol], dayUtc, filters)
              : nyCandleIndicesForDay(candlesBySymbol[symbol], dayUtc, filters),
          },
          {
            sessionWindow: "US_RANGE_ASIA_LONDON_ENTRY",
            rangeSource: "US",
            entrySession: "ASIA_LONDON",
            rangeForSymbol: (symbol) => ranges.us[symbol].get(previousUtcDateKey(dayUtc)),
        ...
        const exec = executePlannedTrades(
          plans,
          state[variant.key].balance,
          state[variant.key].tradeId,
        );
        state[variant.key].balance = exec.endingBalance;
        state[variant.key].tradeId = exec.nextTradeId;
        state[variant.key].trades.push(...exec.closed);
        for (const trade of exec.closed) {
          weeklyEntries[variant.key][trade.symbol as CoreSymbol] += 1;
        }
        if (state[variant.key].balance > state[variant.key].peak) {
          state[variant.key].peak = state[variant.key].balance;
        } else if (state[variant.key].peak > 0) {
          const dd = ((state[variant.key].peak - state[variant.key].balance) / state[variant.key].peak) * 100;
          state[variant.key].maxDd = Math.max(state[variant.key].maxDd, dd);
        }
      }
    }
  }

  const baselineTrades = [...state.baseline.trades].sort((a, b) => Date.parse(a.entry_time_utc) - Date.parse(b.entry_time_utc));
  const doubleTrades = [...state.double.trades].sort((a, b) => Date.parse(a.entry_time_utc) - Date.parse(b.entry_time_utc));

  const baselineTotals = computeStrategyTotals(
    baselineTrades,
    STARTING_BALANCE_USD,
    state.baseline.balance,
    state.baseline.maxDd,
    weekOpens.length,
  );
  const doubleTotals = computeStrategyTotals(
    doubleTrades,
    STARTING_BALANCE_USD,
    state.double.balance,
    state.double.maxDd,
    weekOpens.length,
  );

  const baselinePnlUsd = baselineTrades.reduce((sum, trade) => sum + trade.pnl_usd, 0);
  const doublePnlUsd = doubleTrades.reduce((sum, trade) => sum + trade.pnl_usd, 0);

  const baselinePf = computeProfitFactor(baselineTrades);
  const doublePf = computeProfitFactor(doubleTrades);

  const baselineAsia = summarizeSessionWindow(baselineTrades, "ASIA_LONDON_RANGE_NY_ENTRY");
  const baselineUs = summarizeSessionWindow(baselineTrades, "US_RANGE_ASIA_LONDON_ENTRY");
  const doubleAsia = summarizeSessionWindow(doubleTrades, "ASIA_LONDON_RANGE_NY_ENTRY");
  const doubleUs = summarizeSessionWindow(doubleTrades, "US_RANGE_ASIA_LONDON_ENTRY");

  const baselineMap = new Map(baselineTrades.map((trade) => [tradeIdentityKey(trade), trade]));
  const doubleMap = new Map(doubleTrades.map((trade) => [tradeIdentityKey(trade), trade]));
  const newTrades = doubleTrades.filter((trade) => !baselineMap.has(tradeIdentityKey(trade)));
  const lostTrades = baselineTrades.filter((trade) => !doubleMap.has(tradeIdentityKey(trade)));

  console.log("=== V2 + DOUBLE SESSION RESULTS ===");
  console.log(`Weeks tested: ${weekOpens.join(", ")}`);
  console.log("Bias:");
  for (const row of biasRows) {
    console.log(`  ${row.weekOpenUtc}: BTC=${row.btc} ETH=${row.eth}`);
  }
  console.log("");

  console.log("BASELINE (current V2):");
  console.log(`  Trades: ${baselineTotals.trades}`);
  console.log(`  Win rate: ${formatPct(baselineTotals.winRatePct)}`);
  console.log(`  Total PnL USD: ${formatUsd(baselinePnlUsd)}`);
  console.log(`  Max drawdown %: ${baselineTotals.maxDrawdownPct.toFixed(2)}`);
  console.log(`  Profit factor: ${Number.isFinite(baselinePf) ? baselinePf.toFixed(2) : "INF"}`);
  console.log("  By session window:");
  console.log(`    ASIA_LONDON_RANGE_NY: trades=${baselineAsia.trades} WR=${formatPct(baselineAsia.winRatePct)} PnL=${formatUsd(baselineAsia.pnlUsd)}`);
  console.log(`    US_RANGE_ASIA_LONDON: trades=${baselineUs.trades} WR=${formatPct(baselineUs.winRatePct)} PnL=${formatUsd(baselineUs.pnlUsd)}`);
  console.log("");

  console.log("DOUBLE SESSION (extended NY window):");
  console.log(`  Trades: ${doubleTotals.trades}`);
  console.log(`  Win rate: ${formatPct(doubleTotals.winRatePct)}`);
  console.log(`  Total PnL USD: ${formatUsd(doublePnlUsd)}`);
  console.log(`  Max drawdown %: ${doubleTotals.maxDrawdownPct.toFixed(2)}`);
  console.log(`  Profit factor: ${Number.isFinite(doublePf) ? doublePf.toFixed(2) : "INF"}`);
  console.log("  By session window:");
  console.log(`    ASIA_LONDON_RANGE_NY+ASIA: trades=${doubleAsia.trades} WR=${formatPct(doubleAsia.winRatePct)} PnL=${formatUsd(doubleAsia.pnlUsd)}`);
  console.log(`    US_RANGE_ASIA_LONDON: trades=${doubleUs.trades} WR=${formatPct(doubleUs.winRatePct)} PnL=${formatUsd(doubleUs.pnlUsd)}`);
  console.log("");

  printTradeList("NEW TRADES (in double session but not baseline):", newTrades);
  console.log("");
  printTradeList("LOST TRADES (in baseline but not double session):", lostTrades);
}
...
async function mainV2DoubleSessionComparison() {
  loadEnvFromFile();
  if (!process.env.DATABASE_URL) throw new Error("DATABASE_URL missing.");
  ...
  await runV2DoubleSessionComparison({
    weekOpens,
    cotHistory,
    fundingBySymbol,
  });
...
mainV2DoubleSessionComparison()
  .catch((error) => {
    console.error("analyze-bitget-v2-double-session-window failed:", error);
    process.exitCode = 1;
  })
  .finally(async () => {
    try {
