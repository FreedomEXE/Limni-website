...
// Use from+count to avoid the from+to bug that returns 0 bars for recent data
async function fetchAllM5(inst, fromUtc) {
  ...
  for (let page = 0; page < 10; page++) {
    ...
    for (const b of bars) {
      allBars.push({ ts: new Date(b.time).getTime(), high: +b.mid.h, low: +b.mid.l });
    }
    // Move cursor past last bar
    const lastTs = new Date(bars[bars.length - 1].time).getTime();
    cursor = new Date(lastTs + 1000).toISOString();
    if (bars.length < 500) break; // No more data
...
async function main() {
  ...
  for (const { pair, inst, dir, adr, ws, ind } of PAIRS) {
    const bars = await fetchAllM5(inst, ws);
    const t = scan(bars, dir, adr);
    const g = t - ind;
    const st = g === 0 ? "MATCH" : g > 0 ? "+" + g + " ghost" : g + " under";
    res.push({ pair, ind, t, g, st, bars: bars.length });
    if (g === 0) match++;
    total++;
  ...
  console.log("\n\n=== M5 + INDICATOR ADR + CORRECT FETCH ===\n");
  console.log("Pair".padEnd(12), "Ind", "M5", "Gap", "Bars", "Status");
  console.log("-".repeat(55));
  res.forEach(r => console.log(r.pair.padEnd(12), String(r.ind).padEnd(4), String(r.t).padEnd(4), String(r.g).padEnd(5), String(r.bars).padEnd(6), r.st));
  console.log("-".repeat(55));
