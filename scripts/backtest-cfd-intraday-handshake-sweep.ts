...
-----------------------------------------------*/
/**
 * File: backtest-cfd-intraday-handshake-sweep.ts
 *
 * Description:
 * Tests weekly, intraday, and combined handshake gates on top of the
 * 5m CFD MA+BB directional-close trigger.
 *
 * Run:
 *   npx tsx scripts/backtest-cfd-intraday-handshake-sweep.ts
 */
/*-----------------------------------------------
...
type TriggerConfig = {
  id: string;
  bbStdDev: number;
  gateMode: "NONE" | "WEEKLY" | "INTRADAY" | "COMBINED";
  weeklyThresholdPct: number | null;
  intradayThresholdPct: number | null;
};
...
type HandshakeDetail = {
  ...
  intradayWeakAgreementPct: number | null;
  intradayStrongAgreementPct: number | null;
  intradayAgreementPct: number | null;
  passed: boolean;
};
...
type TriggerTrade = {
  variantId: string;
  bbStdDev: number;
  gateMode: TriggerConfig["gateMode"];
  weeklyThresholdPct: number | null;
  intradayThresholdPct: number | null;
  weekOpenUtc: string;
  weekLabel: string;
...
const TRIGGER_CONFIGS: TriggerConfig[] = [
  { id: "no_handshake__bb2_0", bbStdDev: 2.0, gateMode: "NONE", weeklyThresholdPct: null, intradayThresholdPct: null },
  { id: "weekly_60__bb2_0", bbStdDev: 2.0, gateMode: "WEEKLY", weeklyThresholdPct: 60, intradayThresholdPct: null },
  { id: "intraday_50__bb2_0", bbStdDev: 2.0, gateMode: "INTRADAY", weeklyThresholdPct: null, intradayThresholdPct: 50 },
  { id: "intraday_60__bb2_0", bbStdDev: 2.0, gateMode: "INTRADAY", weeklyThresholdPct: null, intradayThresholdPct: 60 },
  { id: "combined_w60_i50__bb2_0", bbStdDev: 2.0, gateMode: "COMBINED", weeklyThresholdPct: 60, intradayThresholdPct: 50 },
  { id: "combined_w60_i60__bb2_0", bbStdDev: 2.0, gateMode: "COMBINED", weeklyThresholdPct: 60, intradayThresholdPct: 60 },
];
...
function inferIntradayCurrencyState(
  pair: PairInfo,
  sharedCurrency: string,
  candles: OhlcCandle[],
  indicators: IndicatorPack,
  ...
  if (lastIndex < 0) return "NEUTRAL";
  const ma = indicators.sma200[lastIndex];
  if (ma === null) return "NEUTRAL";
  const close = candles[lastIndex].close;
  if (close === ma) return "NEUTRAL";
  const pairDirection: Direction = close > ma ? "LONG" : "SHORT";
  return inferCurrencyState(pair, sharedCurrency, pairDirection);
}

function computeIntradayBucketAgreementPct(
  sharedCurrency: string,
  desiredState: CurrencyState,
  bucketPairs: readonly PairInfo[],
  currentPair: PairInfo,
  ...
  indicatorsByPair: Map<string, Map<number, IndicatorPack>>,
) {
  let eligible = 0;
  let agreeing = 0;
  for (const pairInfo of bucketPairs) {
    ...
    const indicators = indicatorsByPair.get(key)?.get(2.0);
    if (!candles || !indicators) continue;
    const inferred = inferIntradayCurrencyState(pairInfo, sharedCurrency, candles, indicators, entryTs);
    if (inferred === "NEUTRAL") continue;
    eligible += 1;
    if (inferred === desiredState) agreeing += 1;
  }
...
function computeHandshakeDetail(
  ...
  const noGate = config.gateMode === "NONE";
  ...
  if (pairInfo.assetClass === "indices" || pairInfo.assetClass === "crypto") {
    ...
    const intradayPass = bucketPairs.every((row) => {
      const current = pairInfo.pair === row.pair ? pairInfo : ({ ...row, assetClass: pairInfo.assetClass } as PairInfo);
      const key = pairKey(pairInfo.assetClass, row.pair);
      const candles = rawM5ByPair.get(key);
      const indicators = indicatorsByPair.get(key)?.get(2.0);
      if (!candles || !indicators) return false;
      let lastIndex = -1;
      for (let i = 0; i < candles.length; i += 1) {
        const closeTs = candles[i].ts + 5 * 60 * 1000;
      ...
      const close = candles[lastIndex].close;
      return direction === "LONG" ? close > ma : close < ma;
    });
    const passed =
      noGate ||
      (config.gateMode === "WEEKLY" && weeklyPass) ||
      (config.gateMode === "INTRADAY" && intradayPass) ||
      (config.gateMode === "COMBINED" && weeklyPass && intradayPass);
    return {
      ...
      intradayWeakAgreementPct: null,
      intradayStrongAgreementPct: null,
      intradayAgreementPct: intradayPass ? 100 : 0,
      passed,
    };
  }
  ...
  const weeklyStrongAgreementPct = computeBucketAgreementPct(strongCurrency, "STRONG", strongBucket, pairInfo, weekMap);
  const weeklyWeakAgreementPct = computeBucketAgreementPct(weakCurrency, "WEAK", weakBucket, pairInfo, weekMap);
  const intradayStrongAgreementPct = computeIntradayBucketAgreementPct(
    strongCurrency,
    "STRONG",
    strongBucket,
    pairInfo,
  ...
  const intradayWeakAgreementPct = computeIntradayBucketAgreementPct(
    weakCurrency,
    "WEAK",
    weakBucket,
    pairInfo,
  ...
  const intradayAvailable = [intradayStrongAgreementPct, intradayWeakAgreementPct].filter((v): v is number => v !== null);
  const weeklyAgreementPct = weeklyAvailable.length === 0 ? 0 : round(safeAverage(weeklyAvailable), 2);
  const intradayAgreementPct = intradayAvailable.length === 0 ? 0 : round(safeAverage(intradayAvailable), 2);

  const passed =
    noGate ||
    (config.gateMode === "WEEKLY" && weeklyAgreementPct >= (config.weeklyThresholdPct ?? 0)) ||
    (config.gateMode === "INTRADAY" && intradayAgreementPct >= (config.intradayThresholdPct ?? 0)) ||
    (config.gateMode === "COMBINED" &&
      weeklyAgreementPct >= (config.weeklyThresholdPct ?? 0) &&
      intradayAgreementPct >= (config.intradayThresholdPct ?? 0));
  ...
  return {
    ...
    weeklyAgreementPct,
    intradayWeakAgreementPct,
    intradayStrongAgreementPct,
    intradayAgreementPct,
    passed,
  };
}
...
async function main() {
  ...
  for (const config of TRIGGER_CONFIGS) {
      ...
      for (const pairInfo of pairUniverse) {
        ...
        for (const sessionWindow of sessionWindows) {
          ...
          trades.push({
            ...
            intradayThresholdPct: config.intradayThresholdPct,
            weekOpenUtc,
    ...
    const weeklyAgreementValues = trades
      .map((trade) => trade.handshake.weeklyAgreementPct)
      .filter((value): value is number => value !== null);
    const intradayAgreementValues = trades
      .map((trade) => trade.handshake.intradayAgreementPct)
      .filter((value): value is number => value !== null);
    ...
    variantOutputs.push({
      ...
      bucketStats: {
        weeklyAverageAgreementPct: round(safeAverage(weeklyAgreementValues), 2),
        weeklyMedianAgreementPct: round(median(weeklyAgreementValues), 2),
        weeklyDistribution: histogram(weeklyAgreementValues),
        intradayAverageAgreementPct: round(safeAverage(intradayAgreementValues), 2),
        intradayMedianAgreementPct: round(median(intradayAgreementValues), 2),
        intradayDistribution: histogram(intradayAgreementValues),
      },
  ...
  const ranking = [...variantOutputs]
    ...
    .map((row, index) => ({
      ...
      intradayThresholdPct: row.intradayThresholdPct,
  ...
  const report = {
    ...
    config: {
      ...
      handshakeScoring:
        "average of strong/weak bucket agreement percentages with current pair excluded; weekly uses weekly bias, intraday uses price vs 200 SMA at entry time",
    ...
    notes: [
      ...
      "Intraday handshake uses the current side of the 200 SMA at entry time for each related pair.",
      "Indices and crypto use fixed all-members-must-agree checks for weekly and intraday gates.",
  ...
  const reportPath = path.join(REPORTS_DIR, `cfd-intraday-handshake-sweep-${timestamp}.json`);
  const latestPath = path.join(REPORTS_DIR, "cfd-intraday-handshake-sweep-latest.json");
  writeFileSync(reportPath, JSON.stringify(report, null, 2), "utf8");
  writeFileSync(latestPath, JSON.stringify(report, null, 2), "utf8");

  console.log("CFD intraday handshake sweep complete.");
  console.log(JSON.stringify({ ranking, reportPath, latestPath, missingPairs: missingPairs.length }, null, 2));
}

main().catch((error) => {
  console.error("backtest-cfd-intraday-handshake-sweep failed:", error);
  process.exitCode = 1;
});
