...
async function main() {
  ...
  const suffix = REPORT_TAG
    ...
    : autoSuffixParts.length > 0
      ...
      : "";
  const jsonPath = `app/reports/katarakti-phase1-backtest-${stamp}${suffix}.json`;
  const mdPath = `app/reports/katarakti-phase1-backtest-${stamp}${suffix}.md`;
  const latestJsonPath = `app/reports/katarakti-phase1-backtest-latest${suffix}.json`;
  const latestMdPath = `app/reports/katarakti-phase1-backtest-latest${suffix}.md`;
...
type RwScenario = {
  sustainedN: number;
  sweepRangeMult: number;
  trades: RwTrade[];
  totalPnlUsd: number;
  winRatePct: number;
  profitFactor: number;
  maxDrawdownPct: number;
  avgRangeWidth: number;
};
...
const RW_REPORT_PATH = path.join(process.cwd(), "app", "reports", "katarakti-v3-rangewidth-latest.txt");
const RW_STARTING_EQUITY_USD = Number(process.env.KATARAKTI_RW_START_EQUITY_USD ?? "10000");
const RW_RISK_PER_TRADE_PCT = Number(process.env.KATARAKTI_RW_RISK_PER_TRADE_PCT ?? "1");
const RW_MAX_CONCURRENT_POSITIONS = Number(process.env.KATARAKTI_RW_MAX_CONCURRENT_POSITIONS ?? "8");
const RW_MAX_PORTFOLIO_RISK_PCT = Number(process.env.KATARAKTI_RW_MAX_PORTFOLIO_RISK_PCT ?? "8");
const RW_FETCH_CONCURRENCY = Number(process.env.KATARAKTI_RW_FETCH_CONCURRENCY ?? "4");
const RW_SWEEP_RANGE_MULT = Number(process.env.SWEEP_RANGE_MULT ?? "0.0");
const RW_HARD_STOP_X = Number(process.env.RW_HARD_STOP_X ?? "1.5");
const RW_BE_TRIGGER_X = Number(process.env.RW_BE_TRIGGER_X ?? "0.5");
const RW_LOCK1_TRIGGER_X = Number(process.env.RW_LOCK1_TRIGGER_X ?? "1.0");
const RW_LOCK1_LOCK_X = Number(process.env.RW_LOCK1_LOCK_X ?? "0.3");
const RW_LOCK2_TRIGGER_X = Number(process.env.RW_LOCK2_TRIGGER_X ?? "1.5");
const RW_LOCK2_LOCK_X = Number(process.env.RW_LOCK2_LOCK_X ?? "0.8");
const RW_TRAIL_ACTIVATE_X = Number(process.env.RW_TRAIL_ACTIVATE_X ?? "2.0");
const RW_TRAIL_OFFSET_X = Number(process.env.RW_TRAIL_OFFSET_X ?? "0.75");
const RW_POINT_VALUE_USD = 1.0;
const RW_DEBUG_COMMODITIES =
  process.env.KATARAKTI_RW_DEBUG_COMMODITIES === "1" ||
  process.env.KATARAKTI_RW_DEBUG_COMMODITIES?.toLowerCase() === "true";
const RW_EXIT_STEP_ORDER: RwExitStep[] = [
  "hard_stop",
  "breakeven_stop",
  "lock1_stop",
  "lock2_stop",
...
function parseRwSustainedGrid() {
  const raw = (process.env.SUSTAINED_N ?? "").trim();
  if (!raw) return [15, 30, 45, 60];
  const parsed = raw
    .split(",")
    .map((v) => Number(v.trim()))
    .filter((v) => Number.isFinite(v) && v > 0)
    .map((v) => Math.round(v));
  if (!parsed.length) return [15, 30, 45, 60];
  return Array.from(new Set(parsed)).sort((a, b) => a - b);
}

function detectRwSustainedEntry(params: {
  candles: OhlcCandle[];
  entryIndices: number[];
  direction: SignalDirection;
  rangeLow: number;
  ...
  let inQualifiedSweep = false;
  let streak = 0;
  const minSweepDistance = Math.max(0, sweepRangeMult) * rangeWidth;

  for (const idx of entryIndices) {
    const candle = candles[idx];
    if (!candle) continue;

    const isBeyond = direction === "SHORT"
      ? candle.close > rangeHigh
      : candle.close < rangeLow;
    const sweepDistance = direction === "SHORT"
      ? candle.high - rangeHigh
      : rangeLow - candle.low;
    const sweepQualified = sweepDistance > 0 && sweepDistance >= minSweepDistance;

    if (!inQualifiedSweep) {
      if (sweepQualified && isBeyond) {
        inQualifiedSweep = true;
        streak = 1;
        if (streak >= sustainedN) return idx;
      }
      continue;
    }
...
function analyzeSweepStreaks(params: {
  candles: OhlcCandle[];
  entryIndices: number[];
  direction: SignalDirection;
  rangeLow: number;
  ...
  rangeWidth: number;
  sweepRangeMult: number;
  sustainedGrid: number[];
}) {
  const { candles, entryIndices, direction, rangeLow, rangeHigh, rangeWidth, sweepRangeMult, sustainedGrid } = params;
  const minSweepDistance = Math.max(0, sweepRangeMult) * rangeWidth;
  const streaks: number[] = [];

  for (let pos = 0; pos < entryIndices.length; pos += 1) {
    const idx = entryIndices[pos]!;
    const candle = candles[idx];
    if (!candle) continue;
    const isBeyond = direction === "SHORT" ? candle.close > rangeHigh : candle.close < rangeLow;
    const sweepDistance = direction === "SHORT" ? candle.high - rangeHigh : rangeLow - candle.low;
    if (!(sweepDistance > 0 && sweepDistance >= minSweepDistance && isBeyond)) continue;

    let streak = 1;
    for (let nextPos = pos + 1; nextPos < entryIndices.length; nextPos += 1) {
      const nextCandle = candles[entryIndices[nextPos]!]!;
      const nextBeyond = direction === "SHORT" ? nextCandle.close > rangeHigh : nextCandle.close < rangeLow;
      if (!nextBeyond) break;
      streak += 1;
    }
    streaks.push(streak);
  ...
  const passesByN = new Map<number, number>();
  for (const n of sustainedGrid) {
    passesByN.set(n, streaks.filter((s) => s >= n).length);
  }
  return { sweepCandidates: streaks.length, maxStreak, passesByN };
}

function debugCommoditySessions(params: {
  weekOpenUtc: string;
  pairInfo: PairInfo;
  ...
  const { weekOpenUtc, pairInfo, candlesM1, bias, sweepRangeMult, sustainedGrid } = params;
  if (pairInfo.assetClass !== "commodities") return;
  if (!candlesM1.length) {
    console.log(`[RW-DEBUG] ${weekOpenUtc.slice(0, 10)} ${pairInfo.pair} no M1 candles loaded`);
    return;
  }
  if (bias !== "LONG" && bias !== "SHORT") {
    console.log(`[RW-DEBUG] ${weekOpenUtc.slice(0, 10)} ${pairInfo.pair} bias=${bias} (no directional trading)`);
    return;
  }
  ...
  console.log(`[RW-DEBUG] ${weekOpenUtc.slice(0, 10)} ${pairInfo.pair} bias=${bias} sessions=${sessions.length}`);
  for (const session of sessions) {
    ...
    if (!rangeIndices.length || !entryIndices.length) {
      console.log(`[RW-DEBUG] ${pairInfo.pair} ${session.sessionName} rangeBars=${rangeIndices.length} entryBars=${entryIndices.length} -> skipped`);
      continue;
    }
    ...
    if (!(rangeWidth > 0)) {
      console.log(`[RW-DEBUG] ${pairInfo.pair} ${session.sessionName} invalid range width`);
      continue;
    }

    const sweep = analyzeSweepStreaks({
      candles: candlesM1,
      entryIndices,
      direction,
      rangeLow,
    ...
    const passParts = sustainedGrid.map((n) => `N${n}=${sweep.passesByN.get(n) ?? 0}`).join(" ");
    console.log(
      `[RW-DEBUG] ${pairInfo.pair} ${session.sessionName} rangeHigh=${rangeHigh.toFixed(6)} rangeLow=${rangeLow.toFixed(6)} rangeWidth=${rangeWidth.toFixed(6)} sweepCandidates=${sweep.sweepCandidates} maxStreak=${sweep.maxStreak} ${passParts}`,
    );
  }
}

function findRangeWidthSignalsForPair(params: {
  assetClass: AssetClass;
  pair: string;
  candlesM1: OhlcCandle[];
  weekOpenUtc: string;
  ...
  const { assetClass, pair, candlesM1, weekOpenUtc, bias, sustainedN, sweepRangeMult } = params;
  if (!candlesM1.length) return [] as RwSignal[];
  if (bias !== "LONG" && bias !== "SHORT") return [] as RwSignal[];
  ...
  for (const session of sessions) {
    ...
    const entryIdx = detectRwSustainedEntry({
      candles: candlesM1,
      entryIndices,
      direction,
      rangeLow,
    ...
    const entryCandle = candlesM1[entryIdx]!;
    const rangeKey = `${pair}|${session.sessionName}|${session.rangeStartMs}|${direction}`;
    if (dedup.has(rangeKey)) continue;
    dedup.add(rangeKey);
    ...
    out.push({
      ...
      direction,
      sessionName: session.sessionName,
      rangeStartMs: session.rangeStartMs,
      entryCandleTs: entryCandle.ts,
      entryTimeMs: entryCandle.ts + MINUTE_MS,
  ...
  return out.sort((a, b) => a.entryTimeMs - b.entryTimeMs);
}

function firstIndexAtOrAfter(candles: OhlcCandle[], ts: number) {
  for (let i = 0; i < candles.length; i += 1) {
    if (candles[i]!.ts >= ts) return i;
  }
  return -1;
}

function simulateRwExitH1(params: {
  ...
  const { candlesH1, entryTimeMs, entryPrice, direction, rangeWidth, weekCloseMs, weekEndMs } = params;
  const stopDistance = RW_HARD_STOP_X * rangeWidth;
  let stopPrice = direction === "LONG"
    ? entryPrice - stopDistance
    : entryPrice + stopDistance;
  let activeStep: RwExitStep = "hard_stop";
  let peakPrice = entryPrice;

  const startTs = Math.ceil(entryTimeMs / HOUR_MS) * HOUR_MS;
  const startIdx = firstIndexAtOrAfter(candlesH1, startTs);

  if (startIdx >= 0) {
    for (let i = startIdx; i < candlesH1.length; i += 1) {
      const candle = candlesH1[i]!;
      if (candle.ts >= weekCloseMs || candle.ts >= weekEndMs) break;

      if (direction === "LONG") {
        if (candle.high > peakPrice) peakPrice = candle.high;
        const peakRwX = (peakPrice - entryPrice) / rangeWidth;

        if (peakRwX >= RW_BE_TRIGGER_X && stopPrice < entryPrice) {
          stopPrice = entryPrice;
          activeStep = "breakeven_stop";
        }
        if (peakRwX >= RW_LOCK1_TRIGGER_X) {
          const lockPrice = entryPrice + RW_LOCK1_LOCK_X * rangeWidth;
          if (lockPrice > stopPrice) {
            stopPrice = lockPrice;
            activeStep = "lock1_stop";
          }
        }
        if (peakRwX >= RW_LOCK2_TRIGGER_X) {
          const lockPrice = entryPrice + RW_LOCK2_LOCK_X * rangeWidth;
          if (lockPrice > stopPrice) {
            stopPrice = lockPrice;
            activeStep = "lock2_stop";
          }
        }
        if (peakRwX >= RW_TRAIL_ACTIVATE_X) {
          const trailPrice = peakPrice - RW_TRAIL_OFFSET_X * rangeWidth;
          if (trailPrice > stopPrice) {
            stopPrice = trailPrice;
            activeStep = "trailing_stop";
          }
        }
        ...
        const peakRwX = (entryPrice - peakPrice) / rangeWidth;
  ...
  const fallback = findLastCandleBefore(candlesH1, Math.min(weekCloseMs, weekEndMs));
  if (fallback) {
    return {
      exitPrice: fallback.close,
      exitTimeMs: Math.min(weekCloseMs, fallback.ts + HOUR_MS),
      exitReason: "WEEK_CLOSE" as const,
      exitStep: "week_close" as const,
    };
  }
  ...
  return {
    exitPrice: entryPrice,
    exitTimeMs: weekCloseMs,
    exitReason: "WEEK_CLOSE" as const,
    exitStep: "week_close" as const,
  };
}

function rwEntryEquity(startEquity: number, accepted: RwTrade[], entryTimeMs: number) {
  let equity = startEquity;
  for (const trade of accepted) {
    if (trade.exitTimeMs <= entryTimeMs) equity += trade.netPnlUsd;
  }
...
function rwOpenPositions(accepted: RwTrade[], entryTimeMs: number) {
  return accepted.filter((t) => t.entryTimeMs < entryTimeMs && t.exitTimeMs > entryTimeMs).length;
}

function rwOpenRiskPct(accepted: RwTrade[], entryTimeMs: number) {
  return accepted
    .filter((t) => t.entryTimeMs < entryTimeMs && t.exitTimeMs > entryTimeMs)
    .reduce((sum, t) => sum + t.riskPct, 0);
}

function rwPairHasOpenPosition(
  accepted: RwTrade[],
  assetClass: AssetClass,
  pair: string,
  entryTimeMs: number,
...
function rwStats(trades: RwTrade[]): RwStats {
  const wins = trades.filter((t) => t.netPnlUsd > 0);
  const losses = trades.filter((t) => t.netPnlUsd < 0);
  const grossWins = wins.reduce((sum, t) => sum + t.netPnlUsd, 0);
  const grossLossAbs = losses.reduce((sum, t) => sum + Math.abs(t.netPnlUsd), 0);
  const avgRangeWidth = trades.length
    ? trades.reduce((sum, t) => sum + t.rangeWidth, 0) / trades.length
    : 0;
  return {
    trades: trades.length,
    wins: wins.length,
...
function rwMaxDrawdownPct(startEquity: number, trades: RwTrade[]) {
  const ordered = [...trades].sort((a, b) => a.exitTimeMs - b.exitTimeMs);
  let equity = startEquity;
  let peak = equity;
  let maxDd = 0;
...
function simulateRwWeek(params: {
  weekOpenUtc: string;
  sustainedN: number;
  sweepRangeMult: number;
  startEquityUsd: number;
  ...
  const weekStartMs = DateTime.fromISO(weekOpenUtc, { zone: "utc" }).toMillis();
  const weekEndMs = weekStartMs + 7 * DAY_MS;
  const weekCloseMs = Math.min(weekEndMs, weekStartMs + (4 * DAY_MS) + (21 * HOUR_MS));

  const candidates: RwSignal[] = [];
  for (const pairInfo of pairUniverse) {
    const pKey = pairKey(pairInfo.assetClass, pairInfo.pair);
    const candlesM1 = candlesM1ByPair.get(pKey) ?? [];
    if (!candlesM1.length) continue;
    const bias = resolveBias(weekBiasMaps.universal, pairInfo.assetClass, pairInfo.pair);
    if (bias !== "LONG" && bias !== "SHORT") continue;
    const found = findRangeWidthSignalsForPair({
      assetClass: pairInfo.assetClass,
      pair: pairInfo.pair,
      candlesM1,
      weekOpenUtc,
  ...
  const accepted: RwTrade[] = [];
  const oneTradePerRangeDirection = new Set<string>();

  for (const candidate of candidates) {
    if (candidate.entryTimeMs >= weekEndMs) continue;
    if (oneTradePerRangeDirection.has(candidate.rangeKey)) continue;
    if (rwOpenPositions(accepted, candidate.entryTimeMs) >= RW_MAX_CONCURRENT_POSITIONS) continue;
    if (rwOpenRiskPct(accepted, candidate.entryTimeMs) + RW_RISK_PER_TRADE_PCT > RW_MAX_PORTFOLIO_RISK_PCT) continue;
    if (rwPairHasOpenPosition(accepted, candidate.assetClass, candidate.pair, candidate.entryTimeMs)) continue;

    const stopDistance = RW_HARD_STOP_X * candidate.rangeWidth;
    if (!(stopDistance > 0)) continue;

    const entryEquity = rwEntryEquity(startEquityUsd, accepted, candidate.entryTimeMs);
    if (!(entryEquity > 0)) continue;
    const riskUsd = entryEquity * (RW_RISK_PER_TRADE_PCT / 100);
    const units = riskUsd / (stopDistance * RW_POINT_VALUE_USD);
    if (!(units > 0) || !Number.isFinite(units)) continue;

    const candlesH1 = candlesH1ByPair.get(pairKey(candidate.assetClass, candidate.pair)) ?? [];
    const exit = simulateRwExitH1({
      candlesH1,
      entryTimeMs: candidate.entryTimeMs,
      entryPrice: candidate.entryPrice,
      direction: candidate.direction,
      rangeWidth: candidate.rangeWidth,
      weekCloseMs,
      weekEndMs,
    });
    ...
    const stopPrice = candidate.direction === "LONG"
      ? candidate.entryPrice - stopDistance
      : candidate.entryPrice + stopDistance;
    ...
    oneTradePerRangeDirection.add(candidate.rangeKey);
  }

  accepted.sort((a, b) => a.exitTimeMs - b.exitTimeMs);
  const endingEquity = accepted.reduce((sum, t) => sum + t.netPnlUsd, startEquityUsd);
  return { trades: accepted, endingEquity };
}

function renderRwScenario(params: {
  scenario: RwScenario;
  pairUniverse: PairInfo[];
}) {
  const { scenario, pairUniverse } = params;
  const trades = scenario.trades;
  const lines: string[] = [];
  lines.push("=== KATARAKTI V3 RANGE-WIDTH NORMALIZED ===");
  lines.push(`Config: SUSTAINED_N=${scenario.sustainedN}, SWEEP_RANGE_MULT=${scenario.sweepRangeMult.toFixed(1)}`);
  lines.push("");
  lines.push("Per-asset-class breakdown:");
  lines.push("| Asset Class | Pairs | Trades | WR% | PnL ($) | PF  | Avg RW |");
  lines.push("| --- | ---: | ---: | ---: | ---: | ---: | ---: |");

  const classes: Array<{ label: string; assetClass: AssetClass }> = [
    { label: "FX", assetClass: "fx" },
  ...
  for (const row of classes) {
    const classPairs = pairUniverse.filter((p) => p.assetClass === row.assetClass);
    const classTrades = trades.filter((t) => t.assetClass === row.assetClass);
    const stats = rwStats(classTrades);
    lines.push(`| ${row.label} | ${classPairs.length} | ${stats.trades} | ${stats.winRatePct.toFixed(2)} | ${fmtUsd(stats.totalPnlUsd)} | ${fmtPf(stats.profitFactor)} | ${stats.avgRangeWidth.toFixed(6)} |`);
  }
  lines.push("");

  const pairRows = pairUniverse.map((pairInfo) => {
    const pairTrades = trades.filter((t) => t.assetClass === pairInfo.assetClass && t.pair === pairInfo.pair);
    const stats = rwStats(pairTrades);
    return {
      pair: pairInfo.pair,
      trades: stats.trades,
      wr: stats.winRatePct,
      pnl: stats.totalPnlUsd,
      avgRw: stats.avgRangeWidth,
    };
  });

  const top10 = [...pairRows]
    .filter((r) => r.trades > 0)
    .sort((a, b) => b.pnl - a.pnl)
    .slice(0, 10);
  const bottom5 = [...pairRows]
    .filter((r) => r.trades > 0)
    .sort((a, b) => a.pnl - b.pnl)
    .slice(0, 5);

  lines.push("Top 10 pairs by PnL:");
  lines.push("| Pair | Trades | WR% | PnL ($) | Avg Range Width |");
  lines.push("| --- | ---: | ---: | ---: | ---: |");
  for (const row of top10) {
    lines.push(`| ${row.pair} | ${row.trades} | ${row.wr.toFixed(2)} | ${fmtUsd(row.pnl)} | ${row.avgRw.toFixed(6)} |`);
  }
  if (!top10.length) lines.push("| (none) | 0 | 0.00 | +$0.00 | 0.000000 |");
  lines.push("");
  ...
  if (!bottom5.length) lines.push("| (none) | 0 | 0.00 | +$0.00 | 0.000000 |");
  lines.push("");

  const dist = new Map<RwExitStep, number>();
  for (const step of RW_EXIT_STEP_ORDER) dist.set(step, 0);
  for (const trade of trades) dist.set(trade.exitStep, (dist.get(trade.exitStep) ?? 0) + 1);
  lines.push("Exit step distribution:");
  lines.push("| Step | Count | %   |");
  lines.push("| --- | ---: | ---: |");
  for (const step of RW_EXIT_STEP_ORDER) {
  ...
  lines.push("Overall:");
  lines.push("| N | Trades | WR% | Total PnL | PF | Max DD% | Avg RW (pips/points) |");
  lines.push("| ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
  lines.push(`| ${scenario.sustainedN} | ${trades.length} | ${scenario.winRatePct.toFixed(2)} | ${fmtUsd(scenario.totalPnlUsd)} | ${fmtPf(scenario.profitFactor)} | ${scenario.maxDrawdownPct.toFixed(2)} | ${scenario.avgRangeWidth.toFixed(6)} |`);
  lines.push("");
  return lines;
}

async function mainKataraktiRangeWidth() {
  loadDotEnv();

  const sustainedGrid = parseRwSustainedGrid();
  ...
  const [biasByWeek, seriesRows] = await Promise.all([
    loadBiasMapsByWeek(WEEKS, pairUniverse),
    runWithConcurrency(pairUniverse, RW_FETCH_CONCURRENCY, async (item) => {
      const symbol = oandaSymbol(item.pair, item.assetClass);
      try {
  ...
  if (RW_DEBUG_COMMODITIES) {
    for (const pairInfo of pairUniverse) {
      if (pairInfo.assetClass !== "commodities") continue;
      const pKey = pairKey(pairInfo.assetClass, pairInfo.pair);
      const m1 = candlesM1ByPair.get(pKey) ?? [];
      const h1 = candlesH1ByPair.get(pKey) ?? [];
      console.log(`[RW-DEBUG] loaded ${pairInfo.pair} m1=${m1.length} h1=${h1.length}`);
    }
  }

  if (RW_DEBUG_COMMODITIES) {
    for (const week of WEEKS) {
      const weekBiasMaps = biasByWeek.get(week);
      if (!weekBiasMaps) continue;
      for (const pairInfo of pairUniverse) {
        if (pairInfo.assetClass !== "commodities") continue;
        const pKey = pairKey(pairInfo.assetClass, pairInfo.pair);
        const candlesM1 = candlesM1ByPair.get(pKey) ?? [];
        const bias = resolveBias(weekBiasMaps.universal, pairInfo.assetClass, pairInfo.pair);
        debugCommoditySessions({
          weekOpenUtc: week,
          pairInfo,
          candlesM1,
          bias,
          sweepRangeMult: RW_SWEEP_RANGE_MULT,
          sustainedGrid,
        });
      }
    }
  ...
  const scenarios: RwScenario[] = [];
  for (const sustainedN of sustainedGrid) {
    let equity = RW_STARTING_EQUITY_USD;
    const trades: RwTrade[] = [];
    for (const week of WEEKS) {
      const weekBiasMaps = biasByWeek.get(week);
      if (!weekBiasMaps) continue;
      const sim = simulateRwWeek({
        weekOpenUtc: week,
        sustainedN,
        sweepRangeMult: RW_SWEEP_RANGE_MULT,
        startEquityUsd: equity,
    ...
    scenarios.push({
      ...
      maxDrawdownPct: rwMaxDrawdownPct(RW_STARTING_EQUITY_USD, trades),
      avgRangeWidth: stats.avgRangeWidth,
    });
  }
  ...
  reportLines.push("-----------------------------------------------/");
  reportLines.push("");
  reportLines.push(`Weeks tested: [${WEEKS.map((w) => w.slice(0, 10)).join(", ")}]`);
  reportLines.push(`Start equity: $${RW_STARTING_EQUITY_USD.toFixed(2)}`);
  reportLines.push(`Risk per trade: ${RW_RISK_PER_TRADE_PCT.toFixed(2)}%`);
  reportLines.push(`Max concurrent positions: ${RW_MAX_CONCURRENT_POSITIONS}`);
  reportLines.push(`Max portfolio risk: ${RW_MAX_PORTFOLIO_RISK_PCT.toFixed(2)}%`);
  reportLines.push("");
  reportLines.push("Range-width exit multipliers:");
  reportLines.push(`- RW_HARD_STOP_X=${RW_HARD_STOP_X}`);
  reportLines.push(`- RW_BE_TRIGGER_X=${RW_BE_TRIGGER_X}`);
  reportLines.push(`- RW_LOCK1_TRIGGER_X=${RW_LOCK1_TRIGGER_X}, RW_LOCK1_LOCK_X=${RW_LOCK1_LOCK_X}`);
  reportLines.push(`- RW_LOCK2_TRIGGER_X=${RW_LOCK2_TRIGGER_X}, RW_LOCK2_LOCK_X=${RW_LOCK2_LOCK_X}`);
  reportLines.push(`- RW_TRAIL_ACTIVATE_X=${RW_TRAIL_ACTIVATE_X}, RW_TRAIL_OFFSET_X=${RW_TRAIL_OFFSET_X}`);
  reportLines.push("");

  for (const scenario of scenarios.sort((a, b) => a.sustainedN - b.sustainedN)) {
    reportLines.push(...renderRwScenario({ scenario, pairUniverse }));
  }

  reportLines.push("Combined summary:");
  reportLines.push("| N | Trades | WR% | Total PnL | PF | Max DD% | Avg RW (pips/points) |");
  reportLines.push("| ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
  for (const row of scenarios.sort((a, b) => a.sustainedN - b.sustainedN)) {
    reportLines.push(`| ${row.sustainedN} | ${row.trades.length} | ${row.winRatePct.toFixed(2)} | ${fmtUsd(row.totalPnlUsd)} | ${fmtPf(row.profitFactor)} | ${row.maxDrawdownPct.toFixed(2)} | ${row.avgRangeWidth.toFixed(6)} |`);
  }
  reportLines.push("");

  if (missingPairs.length) {
    reportLines.push("Missing candle series:");
    for (const row of missingPairs) reportLines.push(`- ${row}`);
    reportLines.push("");
  }

  const report = reportLines.join("\n");
  fs.mkdirSync(path.dirname(RW_REPORT_PATH), { recursive: true });
  fs.writeFileSync(RW_REPORT_PATH, report, "utf8");
  console.log(report);
  console.log(`Wrote ${RW_REPORT_PATH}`);

  await getPool().end();
}

mainKataraktiRangeWidth().catch(async (error) => {
  console.error("analyze-katarakti-rangewidth-backtest failed:", error);
  try {
    await getPool().end();
  } catch {
    // ignore
