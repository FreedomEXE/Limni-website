...
-----------------------------------------------*/
/**
 * File: sweep-weekly-dip-threshold-matrix.ts
 *
 * Description:
 * Runs a fallback-only buy-the-dip / sell-the-rally threshold study across
 * the full 36-pair matrix using unique pair/week/direction observations
...
const THRESHOLDS_BY_ASSET_CLASS: Record<AssetClass, readonly number[]> = {
  fx: [0.5, 0.75, 1.0, 1.25, 1.5],
  indices: [0.75, 1.0, 1.25, 1.5, 2.0],
  crypto: [1.0, 1.5, 2.0, 2.5, 3.0],
  commodities: [0.75, 1.0, 1.25, 1.5, 2.0],
};

const REFERENCE_THRESHOLD_PCT = 1.0;
...
type ThresholdStats = {
  ...
  deltaVsOnePctAvgReturnPct: number;
...
type GroupResult = {
  ...
  recommendedAssetClassThresholdPct?: number | null;
};
...
function chooseBalancedThreshold(thresholds: ThresholdStats[]) {
  ...
  return byReturn[0]?.thresholdPct ?? null;
}
...
async function main() {
  ...
  function buildGroupResult(id: string, label: string, assetClass: AssetClass, groupObservations: Observation[]): GroupResult {
    const thresholds = THRESHOLDS_BY_ASSET_CLASS[assetClass];
    const thresholdRows: ThresholdStats[] = thresholds.map((thresholdPct) => {
      let triggeredCount = 0;
      let totalBaselineReturnPct = 0;
      let totalFallbackReturnPct = 0;
      let totalTriggeredImprovementReturnPct = 0;
      const worstMaePcts: number[] = [];
      let hitMinus050Count = 0;
      let hitMinus100Count = 0;

      return {
        thresholdPct,
        sampleCount: groupObservations.length,
        triggeredCount,
        fillRatePct: 0,
        avgBaselineReturnPct: totalBaselineReturnPct,
        avgFallbackReturnPct: totalFallbackReturnPct,
        deltaVsOpenAvgReturnPct: 0,
        deltaVsOnePctAvgReturnPct: 0,
        avgTriggeredImprovementReturnPct: totalTriggeredImprovementReturnPct,
        postFillMae: {
          avgWorstMaePct: 0,
          medianWorstMaePct: 0,
          worstWorstMaePct: 0,
    ...
    return {
      ...
      assetClass,
      sampleCount: groupObservations.length,
      referenceThresholdPct: REFERENCE_THRESHOLD_PCT,
      ...
      confidence: getConfidence(groupObservations.length),
    };
  }

  const assetClassGroups = new Map<AssetClass, Observation[]>();
  for (const assetClass of Object.keys(THRESHOLDS_BY_ASSET_CLASS) as AssetClass[]) {
    assetClassGroups.set(assetClass, observations.filter((row) => row.assetClass === assetClass));
  }
  ...
  async function finalizeGroupResult(group: GroupResult, groupObservations: Observation[]) {
    const thresholdRows = group.thresholds;
    const thresholdRowByKey = new Map<number, ThresholdStats>(
      thresholdRows.map((row) => [row.thresholdPct, row]),
    );
    const worstMaeBuckets = new Map<number, number[]>();
    for (const thresholdPct of THRESHOLDS_BY_ASSET_CLASS[group.assetClass]) {
      worstMaeBuckets.set(thresholdPct, []);
    }
    ...
    for (const observation of groupObservations) {
      ...
      for (const thresholdPct of THRESHOLDS_BY_ASSET_CLASS[group.assetClass]) {
        const row = thresholdRowByKey.get(thresholdPct)!;
        const entryPrice =
        ...
        if (touched) {
          ...
          const maeBucket = worstMaeBuckets.get(thresholdPct)!;
    ...
    const referenceRow = thresholdRowByKey.get(REFERENCE_THRESHOLD_PCT) ?? null;
    ...
    for (const row of thresholdRows) {
      ...
      row.deltaVsOnePctAvgReturnPct = round(
      ...
      const maeBucket = worstMaeBuckets.get(row.thresholdPct) ?? [];
      const avgWorstMaePct =
        maeBucket.length > 0 ? maeBucket.reduce((sum, value) => sum + value, 0) / maeBucket.length : 0;
      const medianWorstMaePct = median(maeBucket);
      const worstWorstMaePct = maeBucket.length > 0 ? Math.min(...maeBucket) : 0;
      row.postFillMae.avgWorstMaePct = round(avgWorstMaePct, 4);
      row.postFillMae.medianWorstMaePct = round(medianWorstMaePct, 4);
      row.postFillMae.worstWorstMaePct = round(worstWorstMaePct, 4);
    ...
    group.leaders.bestByAvgReturn =
      [...thresholdRows].sort((left, right) => right.avgFallbackReturnPct - left.avgFallbackReturnPct)[0]?.thresholdPct ??
      null;
    group.leaders.cleanestPostFill =
      [...thresholdRows].sort((left, right) => {
    ...
    group.leaders.recommendedBalanced = chooseBalancedThreshold(thresholdRows);
  }
  ...
  for (const assetClass of Object.keys(THRESHOLDS_BY_ASSET_CLASS) as AssetClass[]) {
    ...
    const group = buildGroupResult(assetClass, label, assetClass, groupObservations);
  ...
  for (const { symbol, assetClass } of getAllUniverseSymbols()) {
    ...
    const group = buildGroupResult(symbol, symbol, assetClass, groupObservations);
    await finalizeGroupResult(group, groupObservations);
    if (group.sampleCount < MIN_SYMBOL_SAMPLE_FOR_OVERRIDE) {
      group.recommendationSource = "asset_class_fallback";
      group.recommendedAssetClassThresholdPct = assetClassRecommendationById.get(assetClass) ?? null;
      group.leaders.recommendedBalanced = group.recommendedAssetClassThresholdPct;
    } else {
  ...
  const output = {
    ...
    methodology: {
      ...
      thresholdsByAssetClass: THRESHOLDS_BY_ASSET_CLASS,
      referenceThresholdPct: REFERENCE_THRESHOLD_PCT,
      pathAssumption:
        "canonical_daily_high_low_proxy_for_non_intraday_path; useful for first-pass threshold mapping, not final intraday-exact execution research",
      executionRule:
        "LONG waits for -threshold% dip from weekly open; SHORT waits for +threshold% rally from weekly open; if not touched during the week, fallback keeps original week-open entry",
      recommendationRule:
        `highest avg fallback return among thresholds with fill rate >= ${MIN_FILL_RATE_PCT}% and post-fill <= -1% rate <= ${MAX_POST_FILL_MINUS_ONE_PCT}%; otherwise highest avg fallback return overall`,
      symbolOverrideRule: `symbol-specific recommendation requires >= ${MIN_SYMBOL_SAMPLE_FOR_OVERRIDE} observations; otherwise fall back to the asset-class recommendation`,
    },
  ...
  const jsonPath = path.join(reportsDir, "weekly-dip-threshold-matrix-study.json");
  ...
  const lines = [
    ...
    `- Reference threshold: ${REFERENCE_THRESHOLD_PCT.toFixed(2)}%`,
    "- Mode: fallback only",
    ...
    ...assetClassResults.map((row) =>
      `| ${row.label} | ${row.sampleCount} | ${row.leaders.recommendedBalanced?.toFixed(2) ?? "n/a"}% | ${row.leaders.bestByAvgReturn?.toFixed(2) ?? "n/a"}% | ${row.leaders.cleanestPostFill?.toFixed(2) ?? "n/a"}% | ${row.confidence} |`,
    ),
    "",
    ...
    ...symbolResults.map((row) => {
      const assetClassFallback = row.recommendedAssetClassThresholdPct;
      const deltaVsAsset =
        row.leaders.recommendedBalanced !== null && assetClassFallback !== null
          ? round(row.leaders.recommendedBalanced - assetClassFallback, 2)
          : 0;
      return `| ${row.label} | ${row.assetClass} | ${row.sampleCount} | ${row.leaders.recommendedBalanced?.toFixed(2) ?? "n/a"}% | ${row.recommendationSource ?? "n/a"} | ${deltaVsAsset >= 0 ? "+" : ""}${deltaVsAsset.toFixed(2)}% | ${row.confidence} |`;
    }),
    "",
    ...
    ...assetClassResults.flatMap((row) =>
      row.thresholds.map(
        (threshold) =>
          `| ${row.label} | ${threshold.thresholdPct.toFixed(2)}% | ${threshold.fillRatePct.toFixed(2)}% | ${threshold.avgFallbackReturnPct.toFixed(2)}% | ${threshold.deltaVsOpenAvgReturnPct >= 0 ? "+" : ""}${threshold.deltaVsOpenAvgReturnPct.toFixed(2)}% | ${threshold.deltaVsOnePctAvgReturnPct >= 0 ? "+" : ""}${threshold.deltaVsOnePctAvgReturnPct.toFixed(2)}% | ${threshold.postFillMae.hitMinus100Pct.toFixed(2)}% |`,
      ),
    ),
    "",
  ...
  const mdPath = path.join(reportsDir, "weekly-dip-threshold-matrix-study.md");
...
main().catch((error) => {
  console.error("sweep-weekly-dip-threshold-matrix failed:", error);
  process.exitCode = 1;
});
