...
const LITE_REPORT_PATH = path.join(process.cwd(), "app", "reports", "katarakti-lite-ablation-latest.txt");
const LITE_JSON_REPORT_PATH = path.join(process.cwd(), "app", "reports", "katarakti-lite-ablation-latest.json");
...
async function mainLiteAblation() {
  ...
  const selectedLite = [...results]
    .filter((row) => row.variant.entryRule === "reentry")
