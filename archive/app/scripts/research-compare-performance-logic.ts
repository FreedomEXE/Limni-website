...
async function main() {
  loadEnvFromFile();
  const universe = JSON.parse(readFileSync("Local Environment/data/research_universe.json", "utf-8")) as UniverseReport;
  ...
  try {
    const researchSnapRaw = JSON.parse(
      readFileSync("Local Environment/data/research_cot_snapshots.json", "utf-8"),
  ...
  try {
    const researchSentiment = JSON.parse(
      readFileSync("Local Environment/data/research_sentiment_aggregates.json", "utf-8"),
  ...
  const outPath = `app/reports/research-compare-performance-${DateTime.utc().toISODate()}.json`;
