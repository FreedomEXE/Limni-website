...
const ATRV3_REPORT_PATH = path.join(process.cwd(), "app", "reports", "katarakti-v3-atr-exit-latest.txt");
const ATRV3_START_EQUITY_USD = Number(process.env.KATARAKTI_V3_STARTING_EQUITY_USD ?? "10000");
const ATRV3_RISK_PER_TRADE_PCT = Number(process.env.KATARAKTI_V3_RISK_PER_TRADE_PCT ?? "1");
const ATRV3_MAX_CONCURRENT_POSITIONS = Number(process.env.KATARAKTI_V3_MAX_CONCURRENT_POSITIONS ?? "8");
const ATRV3_MAX_PORTFOLIO_RISK_PCT = Number(process.env.KATARAKTI_V3_MAX_PORTFOLIO_RISK_PCT ?? "8");
const ATRV3_FETCH_CONCURRENCY = Number(process.env.KATARAKTI_V3_FETCH_CONCURRENCY ?? "4");
const ATRV3_SWEEP_THRESHOLD_PCT = Number(process.env.KATARAKTI_V3_SWEEP_THRESHOLD_PCT ?? "0.0");

function parseAtrV3SustainedGrid() {
...
function detectAtrV3SustainedEntry(params: {
  candles: OhlcCandle[];
  entryIndices: number[];
  direction: SignalDirection;
  rangeLow: number;
  ...
  const { candles, entryIndices, direction, rangeLow, rangeHigh, sustainedN, sweepThresholdPct } = params;
  ...
  for (const idx of entryIndices) {
    ...
    const beyond = direction === "SHORT" ? candle.close > rangeHigh : candle.close < rangeLow;
    const qualifiedSweep = breachPct >= sweepThresholdPct && breachPct > 0;

    if (!inQualifiedSweep) {
      if (qualifiedSweep && beyond) {
...
function findAtrV3SignalsForPair(params: {
  assetClass: AssetClass;
  pair: string;
  ...
  const { assetClass, pair, candlesM1, weekOpenUtc, bias, sustainedN, sweepThresholdPct } = params;
  if (!candlesM1.length) return [] as AtrV3Signal[];
  if (bias !== "LONG" && bias !== "SHORT") return [] as AtrV3Signal[];

  const direction: SignalDirection = bias;
  const weekStartMs = DateTime.fromISO(weekOpenUtc, { zone: "utc" }).toMillis();
  const weekEndMs = weekStartMs + 7 * DAY_MS;
  const sessions = buildWeekSessions(weekStartMs, weekEndMs);
  const out: AtrV3Signal[] = [];
  ...
  for (const session of sessions) {
    ...
    const entryIdx = detectAtrV3SustainedEntry({
      candles: candlesM1,
      entryIndices,
      direction,
      rangeLow,
...
function findH1EntryIndexForM1Entry(candlesH1: OhlcCandle[], entryCandleTs: number) {
  let idx = -1;
  for (let i = 0; i < candlesH1.length; i += 1) {
    if (candlesH1[i]!.ts <= entryCandleTs) idx = i;
    else break;
  }
  if (idx >= 0) return idx;
  if (candlesH1.length > 0 && candlesH1[0]!.ts > entryCandleTs) return 0;
  return -1;
}

function atrV3EntryEquity(startEquity: number, accepted: AtrV3Trade[], entryTimeMs: number) {
  let equity = startEquity;
  for (const trade of accepted) {
    if (trade.exitTimeMs <= entryTimeMs) equity += trade.netPnlUsd;
  }
...
function atrV3Stats(trades: AtrV3Trade[]): AtrV3Stats {
  const wins = trades.filter((t) => t.netPnlUsd > 0);
  const losses = trades.filter((t) => t.netPnlUsd < 0);
  const grossWins = wins.reduce((sum, t) => sum + t.netPnlUsd, 0);
  const grossLossAbs = losses.reduce((sum, t) => sum + Math.abs(t.netPnlUsd), 0);
...
function atrV3MaxDrawdownPct(startEquity: number, trades: AtrV3Trade[]) {
  const ordered = [...trades].sort((a, b) => a.exitTimeMs - b.exitTimeMs);
  let equity = startEquity;
  let peak = equity;
  let maxDd = 0;
...
function simulateAtrV3Week(params: {
  weekOpenUtc: string;
  sustainedN: number;
  startEquityUsd: number;
  pairUniverse: PairInfo[];
  ...
  for (const pairInfo of pairUniverse) {
    ...
    if (bias !== "LONG" && bias !== "SHORT") continue;
    const found = findAtrV3SignalsForPair({
      assetClass: pairInfo.assetClass,
      pair: pairInfo.pair,
      candlesM1,
      weekOpenUtc,
      bias,
      sustainedN,
      sweepThresholdPct: ATRV3_SWEEP_THRESHOLD_PCT,
    });
  ...
  const accepted: AtrV3Trade[] = [];
  const oneTradePerRangeDirection = new Set<string>();

  for (const candidate of candidates) {
    if (candidate.entryTimeMs >= weekCloseMs) continue;
    if (oneTradePerRangeDirection.has(candidate.rangeKey)) continue;
    if (atrV3OpenPositions(accepted, candidate.entryTimeMs) >= ATRV3_MAX_CONCURRENT_POSITIONS) continue;
    if (atrV3OpenRiskPct(accepted, candidate.entryTimeMs) + ATRV3_RISK_PER_TRADE_PCT > ATRV3_MAX_PORTFOLIO_RISK_PCT) continue;
    if (atrV3PairHasOpenPosition(accepted, candidate.assetClass, candidate.pair, candidate.entryTimeMs)) continue;

    const stopDistance = candidate.entryPrice * (HARD_STOP_PCT / 100);
    if (!(stopDistance > 0)) continue;

    const entryEquity = atrV3EntryEquity(startEquityUsd, accepted, candidate.entryTimeMs);
    if (!(entryEquity > 0)) continue;
    const riskUsd = entryEquity * (ATRV3_RISK_PER_TRADE_PCT / 100);
    const units = riskUsd / stopDistance;
    if (!(units > 0) || !Number.isFinite(units)) continue;

    const candlesH1 = candlesH1ByPair.get(pairKey(candidate.assetClass, candidate.pair)) ?? [];
    const entryIdxH1 = findH1EntryIndexForM1Entry(candlesH1, candidate.entryCandleTs);
    ...
    accepted.push({
      ...
      stopPrice,
      stopDistance,
      riskPct: ATRV3_RISK_PER_TRADE_PCT,
      riskUsd,
      units,
      netPnlUsd,
...
function renderAtrV3Scenario(params: {
  sustainedN: number;
  scenario: AtrV3Scenario;
  pairUniverse: PairInfo[];
}) {
  const { sustainedN, scenario, pairUniverse } = params;
  const trades = scenario.trades;
  const lines: string[] = [];

  lines.push(`=== KATARAKTI V3 + ATR EXIT (SUSTAINED_N=${sustainedN}) ===`);
  ...
  for (const row of classes) {
    ...
    const stats = atrV3Stats(classTrades);
    lines.push(`| ${row.label} | ${classPairs.length} | ${stats.trades} | ${stats.winRatePct.toFixed(2)} | ${fmtUsd(stats.totalPnlUsd)} | ${fmtPf(stats.profitFactor)} |`);
  }
  lines.push("");
  ...
  const pairRows = pairUniverse.map((pairInfo) => {
    ...
    const stats = atrV3Stats(pairTrades);
    return {
      pair: pairInfo.pair,
      trades: stats.trades,
      wr: stats.winRatePct,
  ...
  const top10 = [...pairRows].filter((r) => r.trades > 0).sort((a, b) => b.pnl - a.pnl).slice(0, 10);
  const bottom5 = [...pairRows].filter((r) => r.trades > 0).sort((a, b) => a.pnl - b.pnl).slice(0, 5);
  ...
  lines.push("| N | Trades | WR% | Total PnL | PF | Max DD% | Total Return % |");
  lines.push("| ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
  lines.push(`| ${sustainedN} | ${trades.length} | ${scenario.winRatePct.toFixed(2)} | ${fmtUsd(scenario.totalPnlUsd)} | ${fmtPf(scenario.profitFactor)} | ${scenario.maxDrawdownPct.toFixed(2)} | ${scenario.totalReturnPct >= 0 ? "+" : ""}${scenario.totalReturnPct.toFixed(2)}% |`);
  lines.push("");

  return lines;
}

async function mainKataraktiV3AtrExit() {
  loadDotEnv();

  const sustainedGrid = parseAtrV3SustainedGrid();
  ...
  const [biasByWeek, seriesRows] = await Promise.all([
    loadBiasMapsByWeek(WEEKS, pairUniverse),
    runWithConcurrency(pairUniverse, ATRV3_FETCH_CONCURRENCY, async (item) => {
      const symbol = oandaSymbol(item.pair, item.assetClass);
      try {
  ...
  const scenarios: AtrV3Scenario[] = [];
  for (const sustainedN of sustainedGrid) {
    let equity = ATRV3_START_EQUITY_USD;
    const trades: AtrV3Trade[] = [];

    for (const week of WEEKS) {
      const weekBiasMaps = biasByWeek.get(week);
      if (!weekBiasMaps) continue;
      const sim = simulateAtrV3Week({
        weekOpenUtc: week,
        sustainedN,
        startEquityUsd: equity,
        pairUniverse,
    ...
    scenarios.push({
      ...
      maxDrawdownPct: atrV3MaxDrawdownPct(ATRV3_START_EQUITY_USD, trades),
      totalReturnPct: ATRV3_START_EQUITY_USD > 0 ? (stats.totalPnlUsd / ATRV3_START_EQUITY_USD) * 100 : 0,
    });
  }
  ...
  reportLines.push(`Start equity: $${ATRV3_START_EQUITY_USD.toFixed(2)}`);
  reportLines.push(`Risk per trade: ${ATRV3_RISK_PER_TRADE_PCT.toFixed(2)}%`);
  reportLines.push(`Max concurrent positions: ${ATRV3_MAX_CONCURRENT_POSITIONS}`);
  reportLines.push(`Max portfolio risk: ${ATRV3_MAX_PORTFOLIO_RISK_PCT.toFixed(2)}%`);
  reportLines.push(`Sweep threshold: ${ATRV3_SWEEP_THRESHOLD_PCT.toFixed(2)}% (any breach at 0.00%)`);
  reportLines.push("");
  reportLines.push("ATR exit config (Phase 2 defaults):");
  reportLines.push(`- ATR_PERIOD=${ATR_PERIOD}`);
  reportLines.push(`- ATR_BREAKEVEN_TRIGGER_X=${ATR_BREAKEVEN_TRIGGER_X}`);
  reportLines.push(`- ATR_LOCK1_TRIGGER_X=${ATR_LOCK1_TRIGGER_X}, ATR_LOCK1_LOCK_X=${ATR_LOCK1_LOCK_X}`);
  reportLines.push(`- ATR_LOCK2_TRIGGER_X=${ATR_LOCK2_TRIGGER_X}, ATR_LOCK2_LOCK_X=${ATR_LOCK2_LOCK_X}`);
  reportLines.push(`- ATR_TRAIL_ACTIVATE_X=${ATR_TRAIL_ACTIVATE_X}, ATR_TRAIL_OFFSET_X=${ATR_TRAIL_OFFSET_X}`);
  reportLines.push("- EXIT_MODE=stepped_no_hard_sl");
  reportLines.push("- LOCK_STYLE=atr");
  reportLines.push("");

  for (const scenario of scenarios.sort((a, b) => a.sustainedN - b.sustainedN)) {
    reportLines.push(...renderAtrV3Scenario({
      sustainedN: scenario.sustainedN,
      scenario,
      pairUniverse,
    }));
  }
  ...
  reportLines.push("| N | Trades | WR% | Total PnL | PF | Max DD% | Total Return % |");
  reportLines.push("| ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
  for (const row of scenarios.sort((a, b) => a.sustainedN - b.sustainedN)) {
    reportLines.push(`| ${row.sustainedN} | ${row.trades.length} | ${row.winRatePct.toFixed(2)} | ${fmtUsd(row.totalPnlUsd)} | ${fmtPf(row.profitFactor)} | ${row.maxDrawdownPct.toFixed(2)} | ${row.totalReturnPct >= 0 ? "+" : ""}${row.totalReturnPct.toFixed(2)}% |`);
  }
  reportLines.push("");

  reportLines.push("Comparison vs current Phase 2:");
  reportLines.push("| System | Trades | WR% | Total Return | Max DD% | PF |");
  reportLines.push("| --- | ---: | ---: | ---: | ---: | ---: |");
  reportLines.push("| Phase 2 (current) | 30 | 36.7% | +21.62% | 0.56% | 36.85 |");
  for (const row of scenarios.sort((a, b) => a.sustainedN - b.sustainedN)) {
    reportLines.push(`| V3 N=${row.sustainedN} + ATR exit | ${row.trades.length} | ${row.winRatePct.toFixed(2)}% | ${row.totalReturnPct >= 0 ? "+" : ""}${row.totalReturnPct.toFixed(2)}% | ${row.maxDrawdownPct.toFixed(2)}% | ${fmtPf(row.profitFactor)} |`);
  }
  reportLines.push("");
  ...
  fs.mkdirSync(path.dirname(ATRV3_REPORT_PATH), { recursive: true });
  fs.writeFileSync(ATRV3_REPORT_PATH, report, "utf8");
  console.log(report);
  console.log(`Wrote ${ATRV3_REPORT_PATH}`);

  await getPool().end();
}

mainKataraktiV3AtrExit().catch(async (error) => {
  console.error("analyze-katarakti-v3-atr-exit-backtest failed:", error);
  try {
    await getPool().end();
  } catch {
    // ignore
