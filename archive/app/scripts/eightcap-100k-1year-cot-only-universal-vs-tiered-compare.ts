...
const VERSION_MODELS: Record<Version, PerformanceModel[]> = {
  // COT-only Universal V1 strips sentiment and Antikythera baskets.
  v1: ["blended", "dealer", "commercial"],
  // COT-only Universal V3 strips sentiment and Antikythera; remains dealer + commercial.
  v3: ["dealer", "commercial"],
};

const TIER_SOURCES: Record<Version, PerformanceModel[]> = {
  // COT-only tiered versions vote on COT models only.
  v1: ["blended", "dealer", "commercial"],
  v3: ["dealer", "commercial"],
};
...
async function loadWeekRows(weekOpenUtc: string): Promise<SnapshotModelRow[]> {
  const models: PerformanceModel[] = ["blended", "dealer", "commercial"];
...
function buildTieredTrades(options: {
  ...
  const { rows, sourceModels } = options;
  const sourceMap = new Map<PerformanceModel, Map<string, { direction: Direction; adjusted_percent: number | null }>>();
  for (const model of sourceModels) {
    sourceMap.set(model, new Map<string, { direction: Direction; adjusted_percent: number | null }>());
  }
...
async function main() {
  ...
  if (!Number.isFinite(LOOKBACK_WEEKS) || LOOKBACK_WEEKS <= 0) {
    throw new Error(`Invalid COT_ONLY_LOOKBACK_WEEKS: ${String(LOOKBACK_WEEKS)}`);
  }

  const weeks = await loadCotOnlyWeeks(LOOKBACK_WEEKS);
  if (weeks.length === 0) {
    throw new Error("No COT-only performance snapshot weeks found.");
  }
  ...
  for (const weekOpenUtc of weeks) {
    ...
    const rows = await loadWeekRows(weekOpenUtc);
  ...
  const coverageByVersion: Record<Version, CoverageSummary> = (["v1", "v3"] as const).reduce(
    (acc, version) => {
    ...
    {} as Record<Version, CoverageSummary>,
  );

  const totalsByVersionPricedWeeksOnly: Record<Version, VersionTotals> = (["v1", "v3"] as const).reduce(
    (acc, version) => {
      const rows = rowsOut.filter(
        (row) => row.version === version && (row.universal.priced_trades > 0 || row.tiered.priced_trades > 0),
      );
      acc[version] = {
    ...
    {} as Record<Version, VersionTotals>,
  );
  ...
  const report = {
    ...
    cot_only_definitions: {
      skipped_systems: ["v2 universal", "v2 tiered"],
      skipped_reason: "V2 depends on sentiment; COT-only V2 degenerates to dealer-only and is not comparable.",
      v1_universal: "Raw hedged baskets using blended + dealer + commercial only (sentiment and antikythera excluded).",
      v1_tiered: "Tiered vote across blended + dealer + commercial (3 COT voters).",
      v3_universal: "Raw hedged baskets using dealer + commercial only (sentiment and antikythera_v3 excluded).",
      v3_tiered: "Tiered vote across dealer + commercial only (2 COT voters).",
    },
    tier_logic: {
      v1: "3 voters (blended/dealer/commercial): T1=3/3, T2=2/3, T3=strict directional winner in remaining states.",
      v3: "2 voters (dealer/commercial): T1=2/2, T2=1 directional + 1 neutral, no T3.",
    },
    weekly: rowsOut,
    coverage_by_version: coverageByVersion,
    totals_by_version: totalsByVersion,
    totals_by_version_priced_weeks_only: totalsByVersionPricedWeeksOnly,
    assumptions: [
  ...
  const jsonPath = `app/reports/eightcap-100k-1year-cot-only-universal-vs-tiered-${stamp}.json`;
  const mdPath = `app/reports/eightcap-100k-1year-cot-only-universal-vs-tiered-${stamp}.md`;
  const latestJsonPath = "app/reports/eightcap-100k-1year-cot-only-universal-vs-tiered-latest.json";
  const latestMdPath = "app/reports/eightcap-100k-1year-cot-only-universal-vs-tiered-latest.md";

  fs.writeFileSync(jsonPath, JSON.stringify(report, null, 2), "utf8");
  fs.writeFileSync(latestJsonPath, JSON.stringify(report, null, 2), "utf8");

  const md: string[] = [];
  md.push("# Eightcap 100k - COT-Only Universal vs Tiered (1 Year)");
  md.push("");
  ...
  md.push("");
  md.push("## Scope");
  md.push("- Includes COT-only variants of V1 and V3.");
  md.push("- Excludes V2 (sentiment-dependent).");
  md.push("");
  md.push("## Price Coverage");
  md.push("| Version | Total Weeks | Priced Weeks (Universal) | Priced Weeks (Tiered) | First Priced Week | Last Priced Week |");
  md.push("| --- | ---: | ---: | ---: | --- | --- |");
  for (const version of ["v1", "v3"] as const) {
    const c = coverageByVersion[version];
    md.push(
      `| ${version.toUpperCase()} | ${c.total_weeks} | ${c.priced_weeks_universal} | ${c.priced_weeks_tiered} | ${c.first_priced_week ?? "-"} | ${c.last_priced_week ?? "-"} |`,
    );
  }
  md.push("");
  ...
  md.push("");
  md.push("## Totals By Version (Priced Weeks Only)");
  for (const version of ["v1", "v3"] as const) {
    const t = totalsByVersionPricedWeeksOnly[version];
    md.push("");
    md.push(`### ${version.toUpperCase()}`);
    md.push("| Mode | PnL | Return (arith) | Return (compounded) | Margin Used | Trades | Win Rate | Avg/Trade |");
    md.push("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
