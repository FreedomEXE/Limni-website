...
type CombinedTieredWeekRow = {
  week_open_utc: string;
  week_label: string;
  stacked: SystemEval;
  dedup_weighted: SystemEval;
  stacked_tier_counts: { tier1: number; tier2: number; tier3: number };
  dedup_trade_counts: {
    total_pairs: number;
    dedup_pairs: number;
    skipped_ties: number;
...
function evaluateUniversal(options: {
  ...
  for (const pair of plannedPairs) {
    ...
    for (const leg of pair.legs) {
      ...
      const tradePnl = adjusted * (move1pct * legScale);
      pnl += tradePnl;
      priced += 1;
...
function evaluateTiered(options: {
  ...
  for (const trade of options.trades) {
    ...
    const marginPerLeg = toNum(lotRow?.margin_required);
    const move1pct = toNum(lotRow?.move_1pct_usd) ?? toNum(lotRow?.move_1pct_per_lot_usd);
    const legScale = effectiveLotScale(lotRow, options.accountScale, w);

    if (marginPerLeg && marginPerLeg > 0) {
      margin += marginPerLeg * legScale;
    }
    if (trade.adjusted_percent === null || !move1pct || move1pct <= 0) continue;

    const tradePnl = trade.adjusted_percent * (move1pct * legScale);
    pnl += tradePnl;
    priced += 1;
    if (tradePnl > 0) wins += 1;
  }
...
function evaluateWeightedDirectionTrades(options: {
  trades: WeightedDirectionTrade[];
  lotMapRows: LotMapRow[];
  accountScale: number;
}): SystemEval {
  let trades = 0;
  ...
  for (const trade of options.trades) {
    if (!Number.isFinite(trade.weight) || trade.weight <= 0) continue;
    trades += 1;

    const lotRow = findLotMapEntry(options.lotMapRows, trade.symbol);
    const marginPerLeg = toNum(lotRow?.margin_required);
    const move1pct = toNum(lotRow?.move_1pct_usd) ?? toNum(lotRow?.move_1pct_per_lot_usd);
    const legScale = effectiveLotScale(lotRow, options.accountScale, trade.weight);
...
function buildDedupWeightedCombinedTiered(options: {
  v1: TieredTrade[];
  v2: TieredTrade[];
  v3: TieredTrade[];
}) {
  const byPair = new Map<string, Array<TieredTrade & { version: Version }>>();
  for (const [version, trades] of [
    ["v1", options.v1],
    ["v2", options.v2],
    ["v3", options.v3],
  ] as const) {
    for (const trade of trades) {
      const key = pairKey(trade.assetClass, trade.symbol);
      if (!byPair.has(key)) byPair.set(key, []);
      byPair.get(key)!.push({ ...trade, version });
    }
  }

  const dedupTrades: WeightedDirectionTrade[] = [];
  let skippedTies = 0;
  let weightedSum = 0;
  ...
  for (const [key, entries] of byPair.entries()) {
    ...
    for (const entry of entries) {
      ...
      if (entry.direction === "SHORT") shortCount += 1;
    }
    if (longCount === shortCount) {
      skippedTies += 1;
      continue;
    }
    const winningDirection: "LONG" | "SHORT" = longCount > shortCount ? "LONG" : "SHORT";
    const supportCount = Math.max(longCount, shortCount);
    const conflictCount = Math.min(longCount, shortCount);
    weightedSum += supportCount;

    let rawPercent: number | null = null;
    for (const entry of entries) {
      if (typeof entry.adjusted_percent !== "number") continue;
      rawPercent = entry.direction === "LONG" ? entry.adjusted_percent : -entry.adjusted_percent;
      break;
    }
    const adjustedPercent =
      rawPercent === null ? null : winningDirection === "LONG" ? rawPercent : -rawPercent;
  ...
  dedupTrades.sort((a, b) =>
    a.assetClass === b.assetClass ? a.symbol.localeCompare(b.symbol) : a.assetClass.localeCompare(b.assetClass),
  );
  ...
  return {
    ...
    stats: {
      ...
      dedup_pairs: dedupTrades.length,
      skipped_ties: skippedTies,
...
function addSystemEvals(rows: SystemEval[]): SystemEval {
  const totals = rows.reduce(
    (acc, row) => {
      acc.trades += row.trades;
      acc.priced_trades += row.priced_trades;
  ...
  return {
    ...
    margin_used_usd: round(totals.margin_used_usd),
    pnl_usd: round(totals.pnl_usd),
    return_pct_on_100k: round((totals.pnl_usd / TARGET_ACCOUNT_SIZE_USD) * 100, 4),
  };
}

function computeStaticDrawdown(weeklyRows: Array<{ pnl_usd: number }>) {
  let cumulative = 0;
  let peak = 0;
  let maxDd = 0;
  ...
  return {
    max_drawdown_usd: round(maxDd),
    max_drawdown_pct_of_start: round((maxDd / TARGET_ACCOUNT_SIZE_USD) * 100, 4),
  };
}
...
async function main() {
  ...
  const combinedTieredWeekly: CombinedTieredWeekRow[] = [];
  ...
  for (const weekOpenUtc of WEEKS) {
    ...
    const weekRows = new Map<Version, VersionWeekRow>();
    const weekTieredTrades = new Map<Version, TieredTrade[]>();
    ...
    for (const version of ["v1", "v2", "v3"] as const) {
      ...
      weekRows.set(version, rowsOut[rowsOut.length - 1]!);
      weekTieredTrades.set(version, tieredTrades);
    }

    const v1Row = weekRows.get("v1")!;
    const v2Row = weekRows.get("v2")!;
    const v3Row = weekRows.get("v3")!;
    const stacked = addSystemEvals([v1Row.tiered, v2Row.tiered, v3Row.tiered]);
    const stackedTierCounts = {
      tier1: v1Row.tier_counts.tier1 + v2Row.tier_counts.tier1 + v3Row.tier_counts.tier1,
      tier2: v1Row.tier_counts.tier2 + v2Row.tier_counts.tier2 + v3Row.tier_counts.tier2,
      tier3: v1Row.tier_counts.tier3 + v2Row.tier_counts.tier3 + v3Row.tier_counts.tier3,
    };
    const dedup = buildDedupWeightedCombinedTiered({
      v1: weekTieredTrades.get("v1") ?? [],
      v2: weekTieredTrades.get("v2") ?? [],
      v3: weekTieredTrades.get("v3") ?? [],
    });
    const dedupWeighted = evaluateWeightedDirectionTrades({
      trades: dedup.trades,
      lotMapRows,
      accountScale,
    });
    combinedTieredWeekly.push({
      week_open_utc: weekOpenUtc,
      week_label: weekLabel(weekOpenUtc),
      stacked,
      dedup_weighted: dedupWeighted,
      stacked_tier_counts: stackedTierCounts,
      dedup_trade_counts: dedup.stats,
    });
  }
  ...
  const report = {
    ...
    weekly: rowsOut,
    combined_tiered: {
      weekly: combinedTieredWeekly,
      totals: {
        stacked: aggregateSystem(combinedTieredWeekly.map((row) => row.stacked)),
        dedup_weighted: aggregateSystem(combinedTieredWeekly.map((row) => row.dedup_weighted)),
      },
      static_drawdown: {
        stacked: computeStaticDrawdown(combinedTieredWeekly.map((row) => row.stacked)),
        dedup_weighted: computeStaticDrawdown(combinedTieredWeekly.map((row) => row.dedup_weighted)),
      },
    },
  ...
  const jsonPath = `app/reports/eightcap-3k-5week-floor-clamped-compare-${stamp}.json`;
  const mdPath = `app/reports/eightcap-3k-5week-floor-clamped-compare-${stamp}.md`;
  const latestJsonPath = "app/reports/eightcap-3k-5week-floor-clamped-compare-latest.json";
  const latestMdPath = "app/reports/eightcap-3k-5week-floor-clamped-compare-latest.md";

  fs.writeFileSync(jsonPath, JSON.stringify(report, null, 2), "utf8");
  fs.writeFileSync(latestJsonPath, JSON.stringify(report, null, 2), "utf8");

  const md: string[] = [];
  md.push("# Eightcap 3k - Universal vs Tiered (5 Weeks, MT5 0.01 Floor-Clamped)");
  md.push("");
  ...
  md.push("");
  md.push("## Combined Tiered (V1 + V2 + V3)");
  md.push("| Mode | PnL | Return (arith) | Return (compounded) | Margin Used | Trades | Win Rate | Avg/Trade | Static Max DD |");
  md.push("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
  md.push(`| Stacked (independent) | ${fmtUsd(report.combined_tiered.totals.stacked.pnl_usd)} | ${fmtPct(report.combined_tiered.totals.stacked.arithmetic_return_pct_on_100k)} | ${fmtPct(report.combined_tiered.totals.stacked.compounded_return_pct_on_100k)} | ${fmtUsd(report.combined_tiered.totals.stacked.margin_used_usd)} | ${report.combined_tiered.totals.stacked.priced_trades} | ${report.combined_tiered.totals.stacked.win_rate.toFixed(2)}% | ${fmtPct(report.combined_tiered.totals.stacked.avg_return_per_priced_trade_pct)} | ${fmtPct(report.combined_tiered.static_drawdown.stacked.max_drawdown_pct_of_start)} |`);
  md.push(`| De-duplicated (vote-weighted) | ${fmtUsd(report.combined_tiered.totals.dedup_weighted.pnl_usd)} | ${fmtPct(report.combined_tiered.totals.dedup_weighted.arithmetic_return_pct_on_100k)} | ${fmtPct(report.combined_tiered.totals.dedup_weighted.compounded_return_pct_on_100k)} | ${fmtUsd(report.combined_tiered.totals.dedup_weighted.margin_used_usd)} | ${report.combined_tiered.totals.dedup_weighted.priced_trades} | ${report.combined_tiered.totals.dedup_weighted.win_rate.toFixed(2)}% | ${fmtPct(report.combined_tiered.totals.dedup_weighted.avg_return_per_priced_trade_pct)} | ${fmtPct(report.combined_tiered.static_drawdown.dedup_weighted.max_drawdown_pct_of_start)} |`);

  md.push("");
  md.push("## Combined Tiered Weekly");
  md.push("| Week | Stacked | De-dup Weighted | Margin (Stacked) | Margin (De-dup) | De-dup Pairs | Ties Skipped | Weight Sum |");
  md.push("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
  for (const row of report.combined_tiered.weekly) {
    md.push(`| ${row.week_label} | ${fmtPct(row.stacked.return_pct_on_100k)} | ${fmtPct(row.dedup_weighted.return_pct_on_100k)} | ${fmtUsd(row.stacked.margin_used_usd)} | ${fmtUsd(row.dedup_weighted.margin_used_usd)} | ${row.dedup_trade_counts.dedup_pairs} | ${row.dedup_trade_counts.skipped_ties} | ${row.dedup_trade_counts.weighted_sum} |`);
  }
  ...
  md.push(`- MT5 minimum lot floor applied per symbol/trade leg: ${MT5_MIN_LOT.toFixed(2)} with lot step ${MT5_LOT_STEP.toFixed(2)}.`);
  md.push("");
