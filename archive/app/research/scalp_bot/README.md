...
- `--publish` to copy summary + plots into `app/public/scalp-bot` for the website.
...
Pair universe is parsed from `app/src/lib/cotPairs.ts` to avoid duplication.

## Spread config

Optional per-pair spread overrides can be supplied via `data/spread_config.json`.
See `app/research/scalp_bot/spread_config.example.json` for a starter template. If `data/spread_config.json` is missing, the runner will also look for `app/research/scalp_bot/spread_config.json`.

## Outputs

Default output directory: `app/research/scalp_bot/output`
