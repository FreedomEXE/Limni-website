...
const GATE_ID = "Gate 80: fully-hedged-baseline-validity-normalization-edge-attribution";
const GATE_DATE = "2026-06-30";
const COMMAND = "npm run engine:gate80:fully-hedged-baseline-validity-normalization-edge-attribution";
...
const DEFAULT_ARTIFACT_DIR = "docs/research/gates/gate80/artifacts/gate80-fully-hedged-baseline-validity-normalization-edge-attribution";
const DEFAULT_REPORT_PATH = `docs/research/gates/gate80/GATE80_FULLY_HEDGED_BASELINE_VALIDITY_NORMALIZATION_EDGE_ATTRIBUTION_${GATE_DATE}.md`;
...
const GATE80_FULLY_HEDGED_NO_LIMIT_RULE_ID = "FULLY_HEDGED_LONG_SHORT_28_PAIR_WEEKLY_T100_S020_NO_LIMIT_REFERENCE";
const GATE80_LONG_ONLY_RULE_ID = "LONG_ONLY_28_PAIR_WEEKLY_T100_S020_L3_REFERENCE";
const GATE80_SHORT_ONLY_RULE_ID = "SHORT_ONLY_28_PAIR_WEEKLY_T100_S020_L3_REFERENCE";
const GATE80_RANDOM_SIDE_RULE_ID = "DETERMINISTIC_RANDOM_SIDE_28_PAIR_WEEKLY_T100_S020_L3_REFERENCE";
...
type RuleFamily =
  ...
  | "all_pairs_direction_reference";
type RuleScope = "pair" | "account";
type RuleKind = "weekly_forced_close" | "carry_until_flip" | "grid" | "weekly_side_grid";
type CloseReason =
  | "weekly_forced_close"
  | "target"
  | "flip"
...
type WeeklySideMode = "both_sides" | "long_only" | "short_only" | "deterministic_random";
...
type ReplayRule = {
  ...
  weekly_side_mode: WeeklySideMode | null;
  ...
  return {
    ...
    warehouse_hard_loss_adr:
      ...
      options.flip_policy === "close_small_loss_warehouse_large_loss"
        ...
        : null,
    weekly_side_mode: options.weekly_side_mode ?? null,
...
function ruleSet(): ReplayRule[] {
  ...
  const edgeAttributionCounterfactualBases = auditBaselines.filter((baseline) => baseline.rule_id === GATE77_T100_L3_RULE_ID);
  const counterfactuals = edgeAttributionCounterfactualBases.flatMap((baseline) => (
  ...
  const fullyHedgedReference = baseRule({
    ...
    kind: "weekly_side_grid",
    target_adr: 1,
    pair_reset_limit_per_week: DEFAULT_PAIR_RESET_LIMIT,
    two_sided_grid: true,
    reference_only: true,
    weekly_side_mode: "both_sides",
    review_role: "fully_hedged_long_and_short_every_pair_weekly_execution_edge_diagnostic",
  });
  const fullyHedgedNoLimit = baseRule({
    rule_id: GATE80_FULLY_HEDGED_NO_LIMIT_RULE_ID,
    rule_family: "fully_hedged_reference",
    kind: "weekly_side_grid",
    target_adr: 1,
    pair_reset_limit_per_week: null,
    ...
    reference_only: true,
    weekly_side_mode: "both_sides",
    review_role: "fully_hedged_long_and_short_every_pair_weekly_no_reset_limit_diagnostic_only",
  });
  const longOnly = baseRule({
    rule_id: GATE80_LONG_ONLY_RULE_ID,
    rule_family: "all_pairs_direction_reference",
    kind: "weekly_side_grid",
    target_adr: 1,
    pair_reset_limit_per_week: DEFAULT_PAIR_RESET_LIMIT,
    two_sided_grid: true,
    reference_only: true,
    weekly_side_mode: "long_only",
    review_role: "long_only_all_pairs_weekly_no_candidate_b_direction_diagnostic",
  });
  const shortOnly = baseRule({
    rule_id: GATE80_SHORT_ONLY_RULE_ID,
    rule_family: "all_pairs_direction_reference",
    kind: "weekly_side_grid",
    target_adr: 1,
    pair_reset_limit_per_week: DEFAULT_PAIR_RESET_LIMIT,
    two_sided_grid: true,
    reference_only: true,
    weekly_side_mode: "short_only",
    review_role: "short_only_all_pairs_weekly_no_candidate_b_direction_diagnostic",
  });
  const randomSide = baseRule({
    rule_id: GATE80_RANDOM_SIDE_RULE_ID,
    rule_family: "all_pairs_direction_reference",
    kind: "weekly_side_grid",
    target_adr: 1,
    pair_reset_limit_per_week: DEFAULT_PAIR_RESET_LIMIT,
    two_sided_grid: true,
    reference_only: true,
    weekly_side_mode: "deterministic_random",
    review_role: "deterministic_random_side_all_pairs_weekly_null_diagnostic",
  });

  return [weeklyForced, carryUntilFlip, ...auditBaselines, ...counterfactuals, fullyHedgedReference, fullyHedgedNoLimit, longOnly, shortOnly, randomSide];
}
...
function replayPairRulesForPair(pair: string, rows: ReplayRow[], selectedWeeks: string[], rules: ReplayRule[], statsByRule: Map<string, RuleStats>) {
  ...
  const states: PairRuleState[] = rules
    .filter((rule) => rule.kind !== "weekly_forced_close" && rule.kind !== "weekly_side_grid")
    .map((rule) => ({
      rule,
...
function deterministicRandomSide(row: ReplayRow) {
  const hash = sha256Stable({
    week_open_utc: row.week_open_utc,
    pair: row.pair,
    candidate_row_key: row.candidate_row_key,
    seed: "gate80_deterministic_random_side_v1",
  });
  return Number.parseInt(hash.slice(0, 2), 16) % 2 === 0 ? "LONG" : "SHORT";
}

function sidesForWeeklyRule(rule: ReplayRule, row: ReplayRow): Side[] {
  if (rule.weekly_side_mode === "both_sides") return ["LONG", "SHORT"];
  if (rule.weekly_side_mode === "long_only") return ["LONG"];
  if (rule.weekly_side_mode === "short_only") return ["SHORT"];
  if (rule.weekly_side_mode === "deterministic_random") return [deterministicRandomSide(row)];
  throw new Error(`Missing weekly side mode for ${rule.rule_id}`);
}

function replayWeeklySideGridForPair(pair: string, rows: ReplayRow[], selectedWeeks: string[], rules: ReplayRule[], statsByRule: Map<string, RuleStats>) {
  const weeklySideRules = rules.filter((rule) => rule.kind === "weekly_side_grid");
  if (weeklySideRules.length === 0) return;
  const lastWeek = selectedWeeks.at(-1);

  for (const rule of weeklySideRules) {
    const stats = statsByRule.get(rule.rule_id)!;
    let serial = 0;

    for (const row of rows) {
      let pairWeekOpen = 0;
      for (const side of sidesForWeeklyRule(rule, row)) {
...
function summarizeRule(stats: RuleStats, weeklyRows: WeeklyRow[], topWeeks: Set<string>, top20BaselineAdr: number): AdapterSummaryRow {
  ...
  return {
    ...
    weekly_side_mode: stats.rule.weekly_side_mode,
    fully_hedged_reference: stats.rule.weekly_side_mode === "both_sides",
...
function selectedGate80Rows(adapterRows: AdapterSummaryRow[]) {
  const selected = new Set([
    FOCUS_RULE_ID,
    GATE77_T075_L3_RULE_ID,
    GATE77_T100_L3_RULE_ID,
    GATE77_T125_L3_RULE_ID,
    GATE79_FULLY_HEDGED_RULE_ID,
    GATE80_FULLY_HEDGED_NO_LIMIT_RULE_ID,
    GATE80_LONG_ONLY_RULE_ID,
    GATE80_SHORT_ONLY_RULE_ID,
    GATE80_RANDOM_SIDE_RULE_ID,
    `${GATE77_T100_L3_RULE_ID}__FLIP_LOSER_WAREHOUSE`,
    `${GATE77_T100_L3_RULE_ID}__NO_FLIP_FORCE_DIAGNOSTIC`,
  ]);
  return adapterRows.filter((row) => selected.has(row.rule_id));
}

function weeklyRowsByRule(weeklyRows: WeeklyRow[], ruleId: string) {
  return weeklyRows.filter((row) => row.rule_id === ruleId);
}

function returnDrawdown(row: AdapterSummaryRow) {
  return row.max_equity_drawdown_adr === 0 ? null : round6(row.final_equity_adr / Math.abs(row.max_equity_drawdown_adr));
}
...
function rawComparisonRows(adapterRows: AdapterSummaryRow[], weeklyRows: WeeklyRow[], pairRows: Array<Record<string, unknown>>, currencyRows: Array<Record<string, unknown>>) {
  return selectedGate80Rows(adapterRows).map((row) => {
    const weekly = weeklyRowsByRule(weeklyRows, row.rule_id);
    const values = weekly.map((week) => week.equity_delta_adr);
    const pairWorst = pairRows.filter((candidate) => candidate.rule_id === row.rule_id).sort((left, right) => Number(left.mtm_adr ?? 0) - Number(right.mtm_adr ?? 0))[0];
    const currencyWorst = currencyRows.filter((candidate) => candidate.rule_id === row.rule_id).sort((left, right) => Number(left.mtm_adr ?? 0) - Number(right.mtm_adr ?? 0))[0];
    return {
      rule_id: row.rule_id,
      rule_family: row.rule_family,
      ...
      closed_total_adr: row.closed_total_adr,
      final_equity_adr: row.final_equity_adr,
      max_drawdown_adr: row.max_equity_drawdown_adr,
      return_drawdown: returnDrawdown(row),
      weekly_mtm_profit_factor: row.equity_profit_factor,
      adr_calmar: returnDrawdown(row),
      adr_sharpe_weekly: weeklySharpe(values),
      adr_sortino_weekly: weeklySortino(values),
      weekly_mtm_win_rate: row.mtm_weekly_win_rate,
      monthly_mtm_win_rate: row.mtm_monthly_win_rate,
      worst_1_week_mtm_loss_adr: row.worst_week_loss_adr,
      worst_5_week_mtm_loss_adr: row.worst_5_week_loss_adr,
      worst_13_week_mtm_loss_adr: row.worst_13_week_loss_adr,
      final_open_unrealized_adr: row.final_open_unrealized_adr,
      worst_open_unrealized_adr: row.worst_week_end_open_unrealized_adr,
      flip_loss_adr: row.flip_loss_adr,
      total_fills: row.fill_count,
      resets: row.target_reset_cycles + row.account_reset_events,
      average_active_pair_side_slots_proxy: row.weekly_side_mode === "both_sides" ? 56 : 28,
      max_active_pair_side_slots_proxy: row.weekly_side_mode === "both_sides" ? 56 : 28,
      average_open_inventory_abs_adr: round6(average(weekly.map((week) => Math.abs(week.open_unrealized_adr)))),
      max_open_inventory_abs_adr: round6(Math.max(...weekly.map((week) => Math.abs(week.open_unrealized_adr)), 0)),
      worst_pair_by_mtm: pairWorst?.pair ?? null,
      worst_pair_mtm_adr: pairWorst?.mtm_adr ?? null,
      worst_currency_by_mtm: currencyWorst?.currency ?? null,
      worst_currency_mtm_adr: currencyWorst?.mtm_adr ?? null,
    };
  });
}

function grossSideSlots(row: AdapterSummaryRow) {
  return row.weekly_side_mode === "both_sides" ? 56 : 28;
}

function normalizedComparisonRows(adapterRows: AdapterSummaryRow[]) {
  const t100 = adapterRows.find((row) => row.rule_id === GATE77_T100_L3_RULE_ID);
  if (!t100) return [];
  const t100DrawdownBudget = Math.abs(t100.max_equity_drawdown_adr);
  return selectedGate80Rows(adapterRows).map((row) => {
    const grossSlots = grossSideSlots(row);
    const fillRatioVsT100 = t100.fill_count === 0 ? null : round6(row.fill_count / t100.fill_count);
    const riskScale = Math.abs(row.max_equity_drawdown_adr) === 0 ? null : t100DrawdownBudget / Math.abs(row.max_equity_drawdown_adr);
    return {
      rule_id: row.rule_id,
      raw_final_equity_adr: row.final_equity_adr,
      raw_max_drawdown_adr: row.max_equity_drawdown_adr,
      gross_pair_side_slots_proxy: grossSlots,
      gross_exposure_normalized_to_28_pair_side_slots_adr: round6(row.final_equity_adr * (28 / grossSlots)),
      margin_proxy_denominator_abs_dd_plus_abs_open: round6(Math.abs(row.max_equity_drawdown_adr) + Math.abs(row.worst_week_end_open_unrealized_adr)),
      margin_proxy_return_per_unit: Math.abs(row.max_equity_drawdown_adr) + Math.abs(row.worst_week_end_open_unrealized_adr) === 0
        ? null
        : round6(row.final_equity_adr / (Math.abs(row.max_equity_drawdown_adr) + Math.abs(row.worst_week_end_open_unrealized_adr))),
      fill_count: row.fill_count,
      fill_ratio_vs_t100: fillRatioVsT100,
      final_equity_per_10000_fills_adr: row.fill_count === 0 ? null : round6((row.final_equity_adr / row.fill_count) * 10000),
      risk_budget_scale_to_t100_dd: riskScale === null ? null : round6(riskScale),
      risk_budget_normalized_final_equity_to_t100_dd_adr: riskScale === null ? null : round6(row.final_equity_adr * riskScale),
      normalized_edge_vs_t100_gross_slots_adr: round6(row.final_equity_adr * (28 / grossSlots) - t100.final_equity_adr),
    };
  });
}

function costSensitivityRows(adapterRows: AdapterSummaryRow[]) {
  const t100 = adapterRows.find((row) => row.rule_id === GATE77_T100_L3_RULE_ID);
  const hedged = adapterRows.find((row) => row.rule_id === GATE79_FULLY_HEDGED_RULE_ID);
  return selectedGate80Rows(adapterRows).map((row) => {
    const breakEven = row.fill_count === 0 ? null : row.final_equity_adr / row.fill_count;
    const equalCostStillBeatsT100AtPoint01 = t100 ? row.final_equity_adr - row.fill_count * 0.01 > t100.final_equity_adr - t100.fill_count * 0.01 : null;
    const hedgedVsDirectionalEqualCostCrossover = t100 && hedged && hedged.fill_count !== t100.fill_count
      ? round6((hedged.final_equity_adr - t100.final_equity_adr) / (hedged.fill_count - t100.fill_count))
      : null;
    return {
      rule_id: row.rule_id,
      fill_count: row.fill_count,
      raw_final_equity_adr: row.final_equity_adr,
      break_even_cost_adr_per_fill: breakEven === null ? null : round6(breakEven),
      cost_adr_per_fill_to_reduce_return_25pct: breakEven === null ? null : round6(breakEven * 0.25),
      cost_adr_per_fill_to_reduce_return_50pct: breakEven === null ? null : round6(breakEven * 0.5),
      cost_adr_per_fill_to_make_flat: breakEven === null ? null : round6(breakEven),
      final_equity_after_0_01_adr_per_fill_cost: round6(row.final_equity_adr - row.fill_count * 0.01),
      final_equity_after_0_05_adr_per_fill_cost: round6(row.final_equity_adr - row.fill_count * 0.05),
      equal_0_01_cost_still_beats_directional_t100: equalCostStillBeatsT100AtPoint01,
      hedged_vs_directional_equal_cost_crossover_adr_per_fill: hedgedVsDirectionalEqualCostCrossover,
      cost_read: row.rule_id === GATE79_FULLY_HEDGED_RULE_ID && breakEven !== null && breakEven > 0.1
        ? "large_gross_edge_buffer_but_full_broker_cost_model_required"
        : "preliminary_cost_proxy_only",
    };
  });
}

function equitySmoothnessRows(adapterRows: AdapterSummaryRow[], weeklyRows: WeeklyRow[]) {
  return selectedGate80Rows(adapterRows).map((row) => {
    const weekly = weeklyRowsByRule(weeklyRows, row.rule_id);
    const values = weekly.map((week) => week.equity_delta_adr);
    const losses = values.filter((value) => value < 0).sort((left, right) => left - right);
    const totalLoss = Math.abs(values.filter((value) => value < 0).reduce((sum, value) => sum + value, 0));
    const top5Loss = Math.abs(losses.slice(0, 5).reduce((sum, value) => sum + value, 0));
    return {
      rule_id: row.rule_id,
      final_equity_adr: row.final_equity_adr,
      max_drawdown_adr: row.max_equity_drawdown_adr,
      equity_curve_smoothness_score: row.equity_curve_roughness_adr === 0 ? null : round6(row.final_equity_adr / row.equity_curve_roughness_adr),
      weekly_mtm_volatility_adr: round6(stddev(values)),
      downside_weekly_volatility_adr: round6(stddev(values.filter((value) => value < 0))),
      time_in_drawdown_weeks: row.time_in_drawdown_weeks,
      longest_drawdown_weeks: row.longest_drawdown_weeks,
      recovery_duration_from_max_drawdown_weeks_proxy: row.longest_drawdown_weeks,
      worst_4_week_period_adr: rollingWorst(values, 4),
      worst_13_week_period_adr: rollingWorst(values, 13),
      worst_26_week_period_adr: rollingWorst(values, 26),
      monthly_consistency: row.mtm_monthly_win_rate,
      path_skew_proxy_mean_minus_median: round6(average(values) - (median(values) ?? 0)),
      top5_loss_share_of_total_losses: totalLoss === 0 ? null : round6(top5Loss / totalLoss),
      unresolved_inventory_read: row.final_open_unrealized_adr < 0 || row.worst_week_end_open_unrealized_adr < -500
        ? "open_inventory_visible"
        : "no_material_unresolved_inventory_visible",
    };
  });
}

function edgeAttributionRows(adapterRows: AdapterSummaryRow[]) {
  const t100 = adapterRows.find((row) => row.rule_id === GATE77_T100_L3_RULE_ID);
  const hedged = adapterRows.find((row) => row.rule_id === GATE79_FULLY_HEDGED_RULE_ID);
  const longOnly = adapterRows.find((row) => row.rule_id === GATE80_LONG_ONLY_RULE_ID);
  const shortOnly = adapterRows.find((row) => row.rule_id === GATE80_SHORT_ONLY_RULE_ID);
  const randomSide = adapterRows.find((row) => row.rule_id === GATE80_RANDOM_SIDE_RULE_ID);
  const noFlip = adapterRows.find((row) => row.rule_id === `${GATE77_T100_L3_RULE_ID}__NO_FLIP_FORCE_DIAGNOSTIC`);
  const warehouse = adapterRows.find((row) => row.rule_id === `${GATE77_T100_L3_RULE_ID}__FLIP_LOSER_WAREHOUSE`);
  const rows = [
    { comparison: "candidate_b_directional_t100_l3_vs_fully_hedged_t100_l3", left: t100, right: hedged },
    { comparison: "candidate_b_directional_t100_l3_vs_long_only_all_pairs", left: t100, right: longOnly },
    { comparison: "candidate_b_directional_t100_l3_vs_short_only_all_pairs", left: t100, right: shortOnly },
    { comparison: "candidate_b_directional_t100_l3_vs_deterministic_random_side", left: t100, right: randomSide },
    { comparison: "candidate_b_current_flip_vs_no_flip_force_diagnostic", left: t100, right: noFlip },
    { comparison: "candidate_b_current_flip_vs_warehouse_losing_flip_diagnostic", left: t100, right: warehouse },
  ];
  return rows.map(({ comparison, left, right }) => ({
    comparison,
    left_rule_id: left?.rule_id ?? null,
    right_rule_id: right?.rule_id ?? null,
    left_final_equity_adr: left?.final_equity_adr ?? null,
    right_final_equity_adr: right?.final_equity_adr ?? null,
    left_minus_right_equity_adr: left && right ? round6(left.final_equity_adr - right.final_equity_adr) : null,
    left_max_drawdown_adr: left?.max_equity_drawdown_adr ?? null,
    right_max_drawdown_adr: right?.max_equity_drawdown_adr ?? null,
    left_minus_right_drawdown_adr: left && right ? round6(left.max_equity_drawdown_adr - right.max_equity_drawdown_adr) : null,
    left_flip_loss_adr: left?.flip_loss_adr ?? null,
    right_flip_loss_adr: right?.flip_loss_adr ?? null,
    read:
      !left || !right
        ? "missing_comparison_row"
        : left.final_equity_adr > right.final_equity_adr && left.max_equity_drawdown_adr >= right.max_equity_drawdown_adr
          ? "left_adds_edge_and_not_more_drawdown"
          : left.final_equity_adr > right.final_equity_adr
            ? "left_adds_return_but_not_drawdown"
            : "left_does_not_add_enough_edge",
  }));
}

function implementationValidityRows() {
  return [
    { check: "exact_hedged_rules", value: "T100/S020/L3 weekly bounded long+short on every forced-28 pair; no Candidate B mutation; diagnostic only" },
    { check: "long_side_lifecycle", value: "tracked as independent LONG cycle with its own fills, target resets, terminal reset-limit close, and data-end open inventory" },
    { check: "short_side_lifecycle", value: "tracked as independent SHORT cycle with identical fill, target reset, terminal reset-limit, and data-end inventory logic" },
    { check: "long_short_netting", value: "not netted; long and short cycles are tracked separately and summed only at weekly/account reporting" },
    { check: "realized_unrealized_pnl", value: "same cycleNetPnlAdr math as directional replay; mark price reconstructed from Gate74B warehouse path" },
    { check: "open_inventory_visible", value: "yes; weekly open_unrealized_adr and data_end sample_end events are written" },
    { check: "future_costs", value: "both sides must pay costs in any full broker model; Gate80 uses preliminary per-fill stress only" },
    { check: "margin_exposure", value: "both sides consume gross pair-side exposure; Gate80 reports gross side-slot and margin proxies, not broker margin" },
    { check: "close_event_symmetry", value: "yes; closeCycle is shared by long, short, hedged, and directional rules" },
    { check: "reset_event_symmetry", value: "yes; target reset and terminal reset-limit logic are shared for both sides" },
    { check: "data_end_inventory", value: "included through sample_end close events and weekly open snapshots" },
    { check: "overlap_counting", value: "gross exposure is double-counted for long+short; PnL is not netted; normalized tables expose this explicitly" },
    { check: "mechanical_validity_answer", value: "mechanically_valid_under_gross_no_cost_replay_semantics" },
  ];
}

function gate80AnswerRows(options: {
  rawRows: Array<Record<string, unknown>>;
  normalizedRows: Array<Record<string, unknown>>;
  costRows: Array<Record<string, unknown>>;
  smoothnessRows: Array<Record<string, unknown>>;
  edgeRows: Array<Record<string, unknown>>;
}) {
  const normalizedHedged = options.normalizedRows.find((row) => row.rule_id === GATE79_FULLY_HEDGED_RULE_ID);
  const costHedged = options.costRows.find((row) => row.rule_id === GATE79_FULLY_HEDGED_RULE_ID);
  const smoothHedged = options.smoothnessRows.find((row) => row.rule_id === GATE79_FULLY_HEDGED_RULE_ID);
  const edgeHedged = options.edgeRows.find((row) => row.comparison === "candidate_b_directional_t100_l3_vs_fully_hedged_t100_l3");
  return [
    {
      question: "Is the fully hedged result mechanically valid?",
      answer: "yes_under_gross_no_cost_replay_semantics",
      evidence: "long_short_cycles_not_netted_shared_close_reset_pnl_math_data_end_inventory_visible",
    },
    {
      question: "Does the fully hedged system still beat the directional system after fair exposure normalization?",
      answer: Number(normalizedHedged?.normalized_edge_vs_t100_gross_slots_adr ?? 0) > 0 ? "yes_on_gross_side_slot_and_risk_budget_proxies" : "not_after_normalization",
      evidence: normalizedHedged ? JSON.stringify(normalizedHedged) : "missing_normalized_hedged_row",
    },
    {
      question: "Is the fully hedged edge large enough to plausibly survive real trading friction?",
      answer: Number(costHedged?.break_even_cost_adr_per_fill ?? 0) > 0.1 ? "plausibly_yes_but_requires_full_broker_model" : "fragile_under_preliminary_cost_proxy",
      evidence: costHedged ? JSON.stringify(costHedged) : "missing_cost_row",
    },
    {
      question: "Is the fully hedged equity curve genuinely smoother, or does it hide unresolved inventory?",
      answer: smoothHedged?.unresolved_inventory_read === "no_material_unresolved_inventory_visible" ? "genuinely_smoother_on_gate80_proxy" : "smoother_but_open_inventory_visible",
      evidence: smoothHedged ? JSON.stringify(smoothHedged) : "missing_smoothness_row",
    },
    {
      question: "Does Candidate B add directional value, reduce risk, increase risk, or mainly introduce flip-loss damage?",
      answer: edgeHedged?.read === "left_does_not_add_enough_edge" ? "candidate_b_direction_does_not_beat_non_directional_hedged_harvest_and_introduces_flip_loss" : "candidate_b_directional_edge_still_visible",
      evidence: edgeHedged ? JSON.stringify(edgeHedged) : "missing_edge_row",
    },
    {
      question: "Which family, if any, is realistically closer to Freedom's target?",
      answer: "fully_hedged_harvest_family_is_closer_but_not_promotable_until_cost_margin_validation",
      evidence: "Gate80 target feasibility uses rough linear ADR scaling only; no promotion or risk layer started",
    },
  ];
}
...
function determineVerdict(normalizedRows: Array<Record<string, unknown>>, costRows: Array<Record<string, unknown>>) {
  const hedgedNormalized = normalizedRows.find((row) => row.rule_id === GATE79_FULLY_HEDGED_RULE_ID);
  const hedgedCost = costRows.find((row) => row.rule_id === GATE79_FULLY_HEDGED_RULE_ID);
  if (!hedgedNormalized) return "FAIL_HEDGED_BASELINE_ARTIFACT_OR_ACCOUNTING_DEFECT_NO_PROMOTION";
  const normalizedEdge = Number(hedgedNormalized.normalized_edge_vs_t100_gross_slots_adr ?? 0);
  if (normalizedEdge <= 0) return "PASS_DIRECTIONAL_EDGE_STILL_VALID_AFTER_NORMALIZATION_NO_PROMOTION";
  if (Number(hedgedCost?.break_even_cost_adr_per_fill ?? 0) > 0.1) {
    return "PASS_HEDGED_BASELINE_PROMISING_BUT_COST_MARGIN_VALIDATION_REQUIRED_NO_PROMOTION";
  }
  return "FAIL_NO_CURRENT_EXIT_FAMILY_MEETS_RETURN_DRAWDOWN_TARGET_NO_PROMOTION";
}
...
function renderReport(options: {
  ...
  smoothnessRows: Array<Record<string, unknown>>;
  edgeRows: Array<Record<string, unknown>>;
  validityRows: Array<Record<string, unknown>>;
  ...
  return [
    ...
    "",
    renderTable(options.validityRows, ["check", "value"]),
    "",
    "## Raw Comparison",
    "",
    renderTable(options.rawRows, ["rule_id", "rule_family", "closed_total_adr", "final_equity_adr", "max_drawdown_adr", "return_drawdown", "weekly_mtm_profit_factor", "adr_sharpe_weekly", "adr_sortino_weekly", "weekly_mtm_win_rate", "monthly_mtm_win_rate", "worst_1_week_mtm_loss_adr", "worst_5_week_mtm_loss_adr", "worst_13_week_mtm_loss_adr", "final_open_unrealized_adr", "worst_open_unrealized_adr", "flip_loss_adr", "total_fills", "resets", "worst_pair_by_mtm", "worst_currency_by_mtm"]),
    "",
    "## Exposure And Margin Normalization",
    "",
    renderTable(options.normalizedRows, ["rule_id", "raw_final_equity_adr", "raw_max_drawdown_adr", "gross_pair_side_slots_proxy", "gross_exposure_normalized_to_28_pair_side_slots_adr", "margin_proxy_denominator_abs_dd_plus_abs_open", "margin_proxy_return_per_unit", "fill_count", "fill_ratio_vs_t100", "final_equity_per_10000_fills_adr", "risk_budget_scale_to_t100_dd", "risk_budget_normalized_final_equity_to_t100_dd_adr", "normalized_edge_vs_t100_gross_slots_adr"]),
    "",
    "## Cost Sensitivity Precheck",
    "",
    renderTable(options.costRows, ["rule_id", "fill_count", "raw_final_equity_adr", "break_even_cost_adr_per_fill", "cost_adr_per_fill_to_reduce_return_25pct", "cost_adr_per_fill_to_reduce_return_50pct", "cost_adr_per_fill_to_make_flat", "final_equity_after_0_01_adr_per_fill_cost", "final_equity_after_0_05_adr_per_fill_cost", "equal_0_01_cost_still_beats_directional_t100", "hedged_vs_directional_equal_cost_crossover_adr_per_fill", "cost_read"]),
    "",
    "## Exposure Proxy",
    "",
    renderTable(options.exposureRows, ["rule_id", "rule_family", "average_active_pair_side_slots_proxy", "max_active_pair_side_slots_proxy", "average_open_inventory_abs_adr", "max_open_inventory_abs_adr", "total_fills", "resets", "worst_pair_by_mtm", "worst_currency_by_mtm"]),
    "",
    "## Equity Smoothness",
    "",
    renderTable(options.smoothnessRows, ["rule_id", "final_equity_adr", "max_drawdown_adr", "equity_curve_smoothness_score", "weekly_mtm_volatility_adr", "downside_weekly_volatility_adr", "time_in_drawdown_weeks", "longest_drawdown_weeks", "worst_4_week_period_adr", "worst_13_week_period_adr", "worst_26_week_period_adr", "monthly_consistency", "path_skew_proxy_mean_minus_median", "top5_loss_share_of_total_losses", "unresolved_inventory_read"]),
    "",
    "## Edge Attribution",
    "",
    renderTable(options.edgeRows, ["comparison", "left_rule_id", "right_rule_id", "left_final_equity_adr", "right_final_equity_adr", "left_minus_right_equity_adr", "left_max_drawdown_adr", "right_max_drawdown_adr", "left_minus_right_drawdown_adr", "left_flip_loss_adr", "right_flip_loss_adr", "read"]),
    "",
...
async function main() {
  ...
  for (let pairIndex = 0; pairIndex < manifest.universe_symbols.length; pairIndex += 1) {
    ...
    if (options.logProgress) console.log(`gate80 pair replay pair=${pairIndex + 1}/${manifest.universe_symbols.length} ${pair}`);
    ...
    replayWeeklySideGridForPair(pair, rows, selectedWeeks, pairRules, statsByRule);
  }
  ...
  const rawRows = withHashes(rawComparisonRows(adapterRows, weeklyRows, pairAttributionRows, currencyAttributionRows));
  const normalizedRows = withHashes(normalizedComparisonRows(adapterRows));
  const costRows = withHashes(costSensitivityRows(adapterRows));
  const exposureRows = withHashes(rawRows.map((row) => ({
    rule_id: row.rule_id,
    rule_family: row.rule_family,
    average_active_pair_side_slots_proxy: row.average_active_pair_side_slots_proxy,
    max_active_pair_side_slots_proxy: row.max_active_pair_side_slots_proxy,
    average_open_inventory_abs_adr: row.average_open_inventory_abs_adr,
    max_open_inventory_abs_adr: row.max_open_inventory_abs_adr,
    total_fills: row.total_fills,
    resets: row.resets,
    worst_pair_by_mtm: row.worst_pair_by_mtm,
    worst_currency_by_mtm: row.worst_currency_by_mtm,
  })));
  const smoothnessRows = withHashes(equitySmoothnessRows(adapterRows, weeklyRows));
  const edgeRows = withHashes(edgeAttributionRows(adapterRows));
  const validityRows = withHashes(implementationValidityRows());
  const directAnswerRows = withHashes(gate80AnswerRows({ rawRows, normalizedRows, costRows, smoothnessRows, edgeRows }));
  ...
  const verdict = determineVerdict(normalizedRows, costRows);
  const validation = {
    ...
    required_baseline_rows_present:
      ...
      adapterByRule.has(GATE79_FULLY_HEDGED_RULE_ID) &&
      adapterByRule.has(GATE80_FULLY_HEDGED_NO_LIMIT_RULE_ID) &&
      adapterByRule.has(GATE80_LONG_ONLY_RULE_ID) &&
      adapterByRule.has(GATE80_SHORT_ONLY_RULE_ID) &&
      adapterByRule.has(GATE80_RANDOM_SIDE_RULE_ID),
    ...
    fully_hedged_no_limit_reference_included: adapterByRule.has(GATE80_FULLY_HEDGED_NO_LIMIT_RULE_ID),
    long_only_reference_included: adapterByRule.has(GATE80_LONG_ONLY_RULE_ID),
    short_only_reference_included: adapterByRule.has(GATE80_SHORT_ONLY_RULE_ID),
    deterministic_random_side_reference_included: adapterByRule.has(GATE80_RANDOM_SIDE_RULE_ID),
    ...
    raw_comparison_written: rawRows.length > 0,
    normalized_comparison_written: normalizedRows.length > 0,
    cost_sensitivity_written: costRows.length > 0,
    exposure_margin_proxy_written: exposureRows.length > 0,
    equity_smoothness_written: smoothnessRows.length > 0,
    edge_attribution_written: edgeRows.length > 0,
    implementation_validity_written: validityRows.length > 0,
  };
  const passed =
    ...
    validation.fully_hedged_no_limit_reference_included &&
    validation.long_only_reference_included &&
    validation.short_only_reference_included &&
    validation.deterministic_random_side_reference_included &&
    ...
    validation.preliminary_cost_sensitivity_only &&
    ...
    validation.raw_comparison_written &&
    validation.normalized_comparison_written &&
    validation.cost_sensitivity_written &&
    validation.exposure_margin_proxy_written &&
    validation.equity_smoothness_written &&
    validation.edge_attribution_written &&
    validation.implementation_validity_written;
  const finalVerdict = passed ? verdict : "FAIL_HEDGED_BASELINE_ARTIFACT_OR_ACCOUNTING_DEFECT_NO_PROMOTION";

  const artifacts = {
    scorecardRowsJson: toRepoRelative(path.join(options.artifactDir, "gate80-scorecard.rows.json")),
    scorecardRowsCsv: toRepoRelative(path.join(options.artifactDir, "gate80-scorecard.rows.csv")),
    rawComparisonRowsJson: toRepoRelative(path.join(options.artifactDir, "raw-comparison.rows.json")),
    rawComparisonRowsCsv: toRepoRelative(path.join(options.artifactDir, "raw-comparison.rows.csv")),
    normalizedComparisonRowsJson: toRepoRelative(path.join(options.artifactDir, "normalized-comparison.rows.json")),
    normalizedComparisonRowsCsv: toRepoRelative(path.join(options.artifactDir, "normalized-comparison.rows.csv")),
    costSensitivityRowsJson: toRepoRelative(path.join(options.artifactDir, "cost-sensitivity.rows.json")),
    costSensitivityRowsCsv: toRepoRelative(path.join(options.artifactDir, "cost-sensitivity.rows.csv")),
    exposureMarginProxyRowsJson: toRepoRelative(path.join(options.artifactDir, "exposure-margin-proxy.rows.json")),
    exposureMarginProxyRowsCsv: toRepoRelative(path.join(options.artifactDir, "exposure-margin-proxy.rows.csv")),
    equitySmoothnessRowsJson: toRepoRelative(path.join(options.artifactDir, "equity-smoothness.rows.json")),
    equitySmoothnessRowsCsv: toRepoRelative(path.join(options.artifactDir, "equity-smoothness.rows.csv")),
    edgeAttributionRowsJson: toRepoRelative(path.join(options.artifactDir, "edge-attribution.rows.json")),
    edgeAttributionRowsCsv: toRepoRelative(path.join(options.artifactDir, "edge-attribution.rows.csv")),
    implementationValidityRowsJson: toRepoRelative(path.join(options.artifactDir, "implementation-validity.rows.json")),
    implementationValidityRowsCsv: toRepoRelative(path.join(options.artifactDir, "implementation-validity.rows.csv")),
    ...
    summaryJson: toRepoRelative(path.join(options.artifactDir, "gate80-summary.json")),
    shaIdentity: toRepoRelative(path.join(options.artifactDir, "gate80-sha256.txt")),
  ...
  const commandReceipt = {
    ...
    mode: "implementation_research_fully_hedged_baseline_validity_normalization_edge_attribution_no_promotion",
    audit_contract: {
      ...
      fully_hedged_no_limit_reference_rule_id: GATE80_FULLY_HEDGED_NO_LIMIT_RULE_ID,
      long_only_reference_rule_id: GATE80_LONG_ONLY_RULE_ID,
      short_only_reference_rule_id: GATE80_SHORT_ONLY_RULE_ID,
      deterministic_random_side_reference_rule_id: GATE80_RANDOM_SIDE_RULE_ID,
      preliminary_cost_sensitivity_only: true,
      unbounded_parameter_search_started: false,
    },
  };
  ...
  const summary = {
    ...
    purpose: "determine whether the fully hedged long-short weekly reference is mechanically valid and still superior after exposure, margin-proxy, turnover, cost, smoothness, and edge-attribution checks",
    warehouse: {
    ...
    matrix_scope: {
      ...
      fully_hedged_no_limit_reference_row: GATE80_FULLY_HEDGED_NO_LIMIT_RULE_ID,
      long_only_reference_row: GATE80_LONG_ONLY_RULE_ID,
      short_only_reference_row: GATE80_SHORT_ONLY_RULE_ID,
      deterministic_random_side_reference_row: GATE80_RANDOM_SIDE_RULE_ID,
      fixed_rule_count: rules.length,
    },
    ...
    fully_hedged_no_limit_reference: adapterByRule.get(GATE80_FULLY_HEDGED_NO_LIMIT_RULE_ID) ?? null,
    long_only_reference: adapterByRule.get(GATE80_LONG_ONLY_RULE_ID) ?? null,
    short_only_reference: adapterByRule.get(GATE80_SHORT_ONLY_RULE_ID) ?? null,
    deterministic_random_side_reference: adapterByRule.get(GATE80_RANDOM_SIDE_RULE_ID) ?? null,
    direct_answers: directAnswerRows,
    hedged_benchmark: hedgedRows,
    implementation_validity: validityRows,
    raw_comparison: rawRows,
    normalized_comparison: normalizedRows,
    cost_sensitivity: costRows,
    exposure_margin_proxy: exposureRows,
    equity_smoothness: smoothnessRows,
    edge_attribution: edgeRows,
  ...
  await writeRows(path.join(options.artifactDir, "gate80-scorecard.rows.json"), path.join(options.artifactDir, "gate80-scorecard.rows.csv"), adapterRows);
  await writeRows(path.join(options.artifactDir, "raw-comparison.rows.json"), path.join(options.artifactDir, "raw-comparison.rows.csv"), rawRows);
  await writeRows(path.join(options.artifactDir, "normalized-comparison.rows.json"), path.join(options.artifactDir, "normalized-comparison.rows.csv"), normalizedRows);
  await writeRows(path.join(options.artifactDir, "cost-sensitivity.rows.json"), path.join(options.artifactDir, "cost-sensitivity.rows.csv"), costRows);
  await writeRows(path.join(options.artifactDir, "exposure-margin-proxy.rows.json"), path.join(options.artifactDir, "exposure-margin-proxy.rows.csv"), exposureRows);
  await writeRows(path.join(options.artifactDir, "equity-smoothness.rows.json"), path.join(options.artifactDir, "equity-smoothness.rows.csv"), smoothnessRows);
  await writeRows(path.join(options.artifactDir, "edge-attribution.rows.json"), path.join(options.artifactDir, "edge-attribution.rows.csv"), edgeRows);
  await writeRows(path.join(options.artifactDir, "implementation-validity.rows.json"), path.join(options.artifactDir, "implementation-validity.rows.csv"), validityRows);
  ...
  await writeJson(path.join(options.artifactDir, "gate80-summary.json"), summary);
  ...
  await writeShaManifest(path.join(options.artifactDir, "gate80-sha256.txt"), GATE_ID, COMMAND, [
    { label: "scorecard_rows_json", path: path.join(options.artifactDir, "gate80-scorecard.rows.json") },
    { label: "scorecard_rows_csv", path: path.join(options.artifactDir, "gate80-scorecard.rows.csv") },
    { label: "raw_comparison_rows_json", path: path.join(options.artifactDir, "raw-comparison.rows.json") },
    { label: "raw_comparison_rows_csv", path: path.join(options.artifactDir, "raw-comparison.rows.csv") },
    { label: "normalized_comparison_rows_json", path: path.join(options.artifactDir, "normalized-comparison.rows.json") },
    { label: "normalized_comparison_rows_csv", path: path.join(options.artifactDir, "normalized-comparison.rows.csv") },
    { label: "cost_sensitivity_rows_json", path: path.join(options.artifactDir, "cost-sensitivity.rows.json") },
    { label: "cost_sensitivity_rows_csv", path: path.join(options.artifactDir, "cost-sensitivity.rows.csv") },
    { label: "exposure_margin_proxy_rows_json", path: path.join(options.artifactDir, "exposure-margin-proxy.rows.json") },
    { label: "exposure_margin_proxy_rows_csv", path: path.join(options.artifactDir, "exposure-margin-proxy.rows.csv") },
    { label: "equity_smoothness_rows_json", path: path.join(options.artifactDir, "equity-smoothness.rows.json") },
    { label: "equity_smoothness_rows_csv", path: path.join(options.artifactDir, "equity-smoothness.rows.csv") },
    { label: "edge_attribution_rows_json", path: path.join(options.artifactDir, "edge-attribution.rows.json") },
    { label: "edge_attribution_rows_csv", path: path.join(options.artifactDir, "edge-attribution.rows.csv") },
    { label: "implementation_validity_rows_json", path: path.join(options.artifactDir, "implementation-validity.rows.json") },
    { label: "implementation_validity_rows_csv", path: path.join(options.artifactDir, "implementation-validity.rows.csv") },
    ...
    { label: "summary_json", path: path.join(options.artifactDir, "gate80-summary.json") },
  ...
  console.log(JSON.stringify({
    ...
    fully_hedged_reference: summary.fully_hedged_reference,
    normalized_comparison: normalizedRows,
    cost_sensitivity: costRows,
    edge_attribution: edgeRows,
    direct_answers: directAnswerRows,
