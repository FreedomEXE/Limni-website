...
const GATE_ID = "Gate 77: pair-two-sided-reset-limit-confirmation-and-failure-anatomy";
const GATE_DATE = "2026-06-29";
const COMMAND = "npm run engine:gate77:pair-two-sided-reset-limit-confirmation-failure-anatomy";
...
const DEFAULT_ARTIFACT_DIR = "docs/research/gates/gate77/artifacts/gate77-pair-two-sided-reset-limit-confirmation-failure-anatomy";
const DEFAULT_REPORT_PATH = `docs/research/gates/gate77/GATE77_PAIR_TWO_SIDED_RESET_LIMIT_CONFIRMATION_FAILURE_ANATOMY_${GATE_DATE}.md`;
...
const GATE76_PAIR_LIMIT3_RULE_ID = "PAIR_TWO_SIDED_GRID_T075_S020_STOP_AFTER_3_PAIR_RESETS_WEEK";
const SPACING_ADR = 0.2;
const TWO_SIDED_TARGETS = [0.75, 1.25, 1.5] as const;
const PAIR_RESET_LIMITS = [2, 3, 5] as const;
...
function ruleSet(): ReplayRule[] {
  ...
  const pairLimits = PAIR_RESET_LIMITS.map((limit) => ({
    ...
    account_reset_limit_per_week: null,
    review_role: "gate76_pair_reset_limit_confirmation",
  }));

  const structureChecks = STRUCTURE_CHECKS.map(({ target, limit }) => ({
    rule_id: `PAIR_TWO_SIDED_GRID_T${formatAdr(target)}_S020_STOP_AFTER_${limit}_PAIR_RESETS_WEEK`,
    rule_family: "pair_reset_limit" as const,
    scope: "pair" as const,
    kind: "grid" as const,
    target_adr: target,
    ...
    pair_reset_limit_per_week: limit,
    account_reset_limit_per_week: null,
    review_role: "controlled_pair_reset_limit_target_structure_check",
  }));

  return [...controls, ...twoSided, ...pairLimits, ...structureChecks];
}
...
async function replayAccountRules(options: {
  ...
  for (let weekIndex = 0; weekIndex < options.weeks.length; weekIndex += 1) {
    ...
    if (options.logProgress) console.log(`gate77 account replay week=${weekIndex + 1}/${options.weeks.length} ${week}`);
...
function failureAnatomyRows(options: {
  ...
  const comparisons = [
    {
      role: "gate76_t075_reset_limit_2_vs_same_target_no_limit",
      candidateRuleId: "PAIR_TWO_SIDED_GRID_T075_S020_STOP_AFTER_2_PAIR_RESETS_WEEK",
      baselineRuleId: "PAIR_TWO_SIDED_GRID_TARGET_075_SPACING_020_RESTART_UNTIL_FLIP",
    },
    {
      role: "gate76_t075_reset_limit_3_vs_same_target_no_limit",
      candidateRuleId: GATE76_PAIR_LIMIT3_RULE_ID,
      baselineRuleId: "PAIR_TWO_SIDED_GRID_TARGET_075_SPACING_020_RESTART_UNTIL_FLIP",
    },
    {
      role: "gate76_t075_reset_limit_5_vs_same_target_no_limit",
      candidateRuleId: "PAIR_TWO_SIDED_GRID_T075_S020_STOP_AFTER_5_PAIR_RESETS_WEEK",
      baselineRuleId: "PAIR_TWO_SIDED_GRID_TARGET_075_SPACING_020_RESTART_UNTIL_FLIP",
    },
    {
      role: "structure_t125_reset_limit_3_vs_same_target_no_limit",
      candidateRuleId: "PAIR_TWO_SIDED_GRID_T125_S020_STOP_AFTER_3_PAIR_RESETS_WEEK",
      baselineRuleId: "PAIR_TWO_SIDED_GRID_TARGET_125_SPACING_020_RESTART_UNTIL_FLIP",
    },
  ];
  ...
  return comparisons.map(({ role, candidateRuleId, baselineRuleId }) => {
    ...
    return {
      ...
      baseline_no_limit_rule_id: baselineRuleId,
      ...
      net_equity_delta_vs_no_limit_adr: round6(candidate.final_equity_adr - baseline.final_equity_adr),
      closed_delta_vs_no_limit_adr: round6(candidate.closed_total_adr - baseline.closed_total_adr),
      max_drawdown_improvement_vs_no_limit_adr: round6(candidate.max_equity_drawdown_adr - baseline.max_equity_drawdown_adr),
      final_open_improvement_vs_no_limit_adr: round6(candidate.final_open_unrealized_adr - baseline.final_open_unrealized_adr),
      flip_loss_improvement_vs_no_limit_adr: round6(candidate.flip_loss_adr - baseline.flip_loss_adr),
      worst_week_loss_improvement_vs_no_limit_adr: round6(candidate.worst_week_loss_adr - baseline.worst_week_loss_adr),
      worst_5_week_loss_improvement_vs_no_limit_adr: round6(candidate.worst_5_week_loss_adr - baseline.worst_5_week_loss_adr),
      worst_13_week_loss_improvement_vs_no_limit_adr: round6(candidate.worst_13_week_loss_adr - baseline.worst_13_week_loss_adr),
      top20_winner_adr_candidate: candidateTop20,
      top20_winner_adr_no_limit: baselineTop20,
      top20_winner_retention_loss_vs_no_limit_adr: round6(Math.max(0, baselineTop20 - candidateTop20)),
      top20_winner_retention_delta_vs_no_limit_adr: round6(candidateTop20 - baselineTop20),
      reset_count_delta_vs_no_limit: candidate.target_reset_cycles - baseline.target_reset_cycles,
      fill_count_delta_vs_no_limit: candidate.fill_count - baseline.fill_count,
      expansion_fill_count_delta_vs_no_limit: candidate.favorable_expansion_fill_count - baseline.favorable_expansion_fill_count,
      recovery_rate_delta_vs_no_limit: candidate.negative_cycle_recovery_rate === null || baseline.negative_cycle_recovery_rate === null
        ? null
...
function renderReport(options: {
  ...
  return [
    ...
    "## Adapter Ranking",
    "",
    renderTable(tableRows(options.adapterRows), ["rule_id", "family", "closed", "equity", "eq_pf", "dd", "open", "worst1", "worst5", "worst13", "neg_years", "profitable_week_rate", "top20", "recovery", "flip_loss", "resets", "fills", "adverse", "expansion"]),
    "",
    "## Failure Anatomy",
    "",
    renderTable(options.failureRows, ["comparison_role", "candidate_rule_id", "baseline_no_limit_rule_id", "helped_week_count", "hurt_week_count", "net_equity_delta_vs_no_limit_adr", "closed_delta_vs_no_limit_adr", "max_drawdown_improvement_vs_no_limit_adr", "final_open_improvement_vs_no_limit_adr", "flip_loss_improvement_vs_no_limit_adr", "top20_winner_retention_loss_vs_no_limit_adr"]),
    "",
    "## Interpretation Rule",
    "",
...
async function main() {
  ...
  if (!manifest) throw new Error(`Gate 77 requires completed Gate 74B warehouse manifest: ${manifestId}`);
  ...
  for (let pairIndex = 0; pairIndex < manifest.universe_symbols.length; pairIndex += 1) {
    ...
    if (options.logProgress) console.log(`gate77 pair replay pair=${pairIndex + 1}/${manifest.universe_symbols.length} ${pair}`);
  ...
  const gate76PairLimit3Reproduced = adapterRowsMatch(adapterByRule.get(GATE76_PAIR_LIMIT3_RULE_ID), gate76AdapterByRule.get(GATE76_PAIR_LIMIT3_RULE_ID));
  const validation = {
    ...
    pair_reset_limit_variant_count: PAIR_RESET_LIMITS.length + STRUCTURE_CHECKS.length,
    ...
    gate76_pair_reset_limit3_reproduced: gate76PairLimit3Reproduced,
    gate76_key_rows_reproduced_before_comparison: gate76T150Reproduced && gate76PairLimit3Reproduced,
  ...
  const passed =
    ...
    validation.fixed_rule_count === 11 &&
    !validation.account_level_replay_started &&
    ...
    validation.gate76_key_rows_reproduced_before_comparison &&
  ...
  const verdict = passed
    ...
    : "FAIL_GATE77_PAIR_TWO_SIDED_RESET_LIMIT_CONFIRMATION_FAILURE_ANATOMY";
  ...
  const artifacts = {
    ...
    summaryJson: toRepoRelative(path.join(options.artifactDir, "gate77-summary.json")),
    shaIdentity: toRepoRelative(path.join(options.artifactDir, "gate77-sha256.txt")),
  ...
  const commandReceipt = {
    ...
    mode: "implementation_research_confirmation_failure_anatomy_no_promotion",
    matrix_contract: {
      spacing_adr: SPACING_ADR,
      pair_two_sided_no_limit_targets: TWO_SIDED_TARGETS,
      pair_reset_limits_at_target_075: PAIR_RESET_LIMITS,
      controlled_structure_checks: STRUCTURE_CHECKS,
      account_reset_targets: [],
      unbounded_parameter_search_started: false,
    },
  };
  ...
  const summary = {
    ...
    command: COMMAND,
    verdict,
    purpose: "confirm Gate 76 pair reset-limit behavior and explain failure modes without broad search or promotion",
    warehouse: {
    ...
    baseline_binding: {
      ...
      gate76_key_rows_reproduced_before_comparison: validation.gate76_key_rows_reproduced_before_comparison,
    },
    matrix_scope: {
    ...
    gate76_promising_pair_reset_limit3: adapterByRule.get(GATE76_PAIR_LIMIT3_RULE_ID) ?? null,
    reset_limit_failure_anatomy: failureRows,
  ...
  await writeJson(path.join(options.artifactDir, "gate77-summary.json"), summary);
  await writeText(options.reportPath, renderReport({ summary, adapterRows, failureRows }));
  await writeShaManifest(path.join(options.artifactDir, "gate77-sha256.txt"), GATE_ID, COMMAND, [
    ...
    { label: "summary_json", path: path.join(options.artifactDir, "gate77-summary.json") },
  ...
  console.log(JSON.stringify({
    ...
    gate76_promising_pair_reset_limit3: summary.gate76_promising_pair_reset_limit3,
    artifacts,
  }, null, 2));
}
