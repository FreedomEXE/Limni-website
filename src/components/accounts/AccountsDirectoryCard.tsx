...
import { basketTone, statusTone } from "@/lib/accounts/uiTones";
import type { AccountCard } from "@/lib/accounts/accountsDirectoryTypes";

type AccountsDirectoryCardProps = {
  account: AccountCard;
  deleting: boolean;
  weekParam: string | null;
  viewParam: string | null;
  onDelete: (account: AccountCard) => void;
};

export default function AccountsDirectoryCard({
  account,
  deleting,
  weekParam,
  viewParam,
  onDelete,
}: AccountsDirectoryCardProps) {
  return (
    <div
    ...
    >
      ...
      {account.href ? (
        <Link href={buildAccountCardHref(account.href, weekParam, viewParam)} className="block">
  ...
  return (
    <>
      <div className="flex items-start justify-between gap-4">
        <div>
          <h2 className="text-lg font-semibold text-[var(--foreground)]">{account.label}</h2>
      ...
      <div className="mt-4 grid gap-3 text-sm text-[var(--foreground)] sm:grid-cols-2">
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--muted)]">Equity</p>
          <p className="mt-1 font-semibold">{formatMaybeCurrency(account.equity, account.currency)}</p>
        </div>
        <div>
          <p className="text-xs uppercase tracking-[0.2em] text-[color:var(--muted)]">Weekly PnL</p>
          <p
            className={`mt-1 font-semibold ${
              (account.weekly_pnl_pct ?? 0) >= 0 ? "text-emerald-700" : "text-rose-700"
            }`}
          >
