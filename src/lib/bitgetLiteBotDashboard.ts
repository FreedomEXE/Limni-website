...
-----------------------------------------------*/
/**
 * File: bitgetLiteBotDashboard.ts
 *
 * Description:
 * Read-only data access helpers for the Katarakti Crypto Lite monitoring UI.
...
const BOT_ID = "katarakti_crypto_lite";
...
async function safeQuery<T>(
  ...
  try {
    ...
    console.warn(
      "[bitgetLiteBotDashboard] query failed:",
...
async function safeQueryOne<T>(
  ...
  try {
    ...
    console.warn(
      "[bitgetLiteBotDashboard] queryOne failed:",
      error instanceof Error ? error.message : String(error),
    );
    return null;
  }
...
export async function readBitgetLiteBotStatusData(): Promise<BitgetBotStatusPayload> {
