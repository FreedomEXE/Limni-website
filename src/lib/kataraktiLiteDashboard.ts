...
-----------------------------------------------*/
/**
 * File: kataraktiLiteDashboard.ts
 *
 * Description:
 * Read-only data access helpers for the Katarakti CFD Lite sweep-entry system
...
const BOT_ID = "katarakti_cfd_lite";
...
async function safeQuery<T>(
  ...
  try {
    ...
    console.warn(
      "[kataraktiLiteDashboard] query failed:",
...
async function safeQueryOne<T>(
  ...
  try {
    ...
    console.warn(
      "[kataraktiLiteDashboard] queryOne failed:",
      error instanceof Error ? error.message : String(error),
    );
    return null;
  }
...
export async function readKataraktiLiteStatusData(): Promise<KataraktiStatusPayload> {
