...
function Write-ScanConfig(
  ...
  $report = "Gate99ZZ_AUDCHF_$tag"
  $fromText = $from.ToString("yyyy.MM.dd")
  $toText = $to.ToString("yyyy.MM.dd")
  $content = @"
; Gate 99ZZ one-pair Revma mechanics scan.
...
foreach ($start in $starts) {
  ...
  $folderName = "LimniPortfolioEA_Gate99ZZ_AUDCHF_$tag"
  ...
  Write-Host ("completed {0} done={1} birth={2}/{3}/{4} adds={5}" -f $tag, $done, $scanRows[-1].direction, $scanRows[-1].anchor_location, $scanRows[-1].sleeve, $adds.Count)
}
...
$scanRows | Format-Table tag,from,to,direction,anchor_location,sleeve,add_policy,add_count,first_add_time -AutoSize | Out-String -Width 260
