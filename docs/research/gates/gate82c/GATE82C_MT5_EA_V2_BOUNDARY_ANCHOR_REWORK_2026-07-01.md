...
| Area | Change |
| --- | --- |
| Mode names | Replaced prototype-facing `NO_LIMIT_RAW` / `L3_RAW` with `RAW_V2` / `L3_V2`. |
...
| L3 semantics | `L3_V2` keeps the fully hedged lane's side-local three target-reset cap per symbol-side per canonical week. |
...
1. Compile `LimniBasketHedgeEAAlphaV1.mq5` in MetaEditor.
2. Run a one-pair tester smoke with `RAW_V2`.
...
4. Confirm first fills wait for the Pine-style entry levels rather than opening
   instantly.
5. Confirm `L3_V2` stops each side after three target resets in the active
   canonical week.
