...
## Strength Baseline Lock

Gate 55 locks Friday Strength as execution-sensitive, not universally
directional.

Accepted Strength baseline lock for the ADR Grid research path:
...
- Selected beats fade under canonical 1m ADR Grid by `+323.1345` ADR.
- Selected ADR Grid result: `+1311.8526` ADR, max DD `-698.5889`, R/DD
  `1.8779`, PF `1.2674`.
- Fade ADR Grid result: `+988.7181` ADR, max DD `-653.5674`, R/DD `1.5128`,
  PF `1.1794`.

Required caveat:

- Simple weekly hold points the other way. Fade weekly hold is `+315.1911` ADR,
  while selected weekly hold is `-315.1911` ADR.
- Therefore Friday Strength is not locked as a plain weekly-hold directional
  engine.
- This is not a COT+Strength combination decision and not a final combined
  system selection.
