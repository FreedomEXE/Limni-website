...
Freedom accepted `carry_previous_clp_side` after review because it is simpler and performs better under the legacy ADR Grid matrix scoring path. This is a working CLP tie-break policy for continued raw-number review, not final Signal Model selection or live promotion.
